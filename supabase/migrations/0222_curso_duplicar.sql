-- Duplicar un curso: clon profundo de TODO el material (módulos+clases, examen
-- +secciones+preguntas+opciones, condiciones, encuentros, bibliografía, encuestas),
-- remapeando los FK. NO clona alumnos matriculados ni nada por-alumno (progreso,
-- intentos, asistencias, certificados, respuestas). El clon nace BORRADOR
-- (activo=false, sin publicar) con slug único y título "… (copia)".
--
-- ⚠ NOTA: el INSERT de curso_encuentros de esta versión tenía un bug de scope
-- (faltaba WHERE o.curso_id = p_curso_id → clonaba encuentros de otros cursos).
-- Lo cazó el e2e sintético antes de shippear y se corrige en 0223. Se conserva
-- este archivo tal cual se aplicó para que el historial repo == remoto.
...
BEGIN
  ...
  SELECT gen_random_uuid(), m.new_id, c.orden, c.titulo, c.descripcion, c.tipo, c.youtube_url, c.zoom_url, c.zoom_fecha_hora,
  ...
  -- 9) encuentros (remap condicion_id; NULL las salas Zoom/Webex → se crean de nuevo)
  --    BUG (corregido en 0223): falta WHERE o.curso_id = p_curso_id.
