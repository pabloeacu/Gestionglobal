-- ============================================================================
-- Migration: 0105_gestor_uploads_bucket_alerta_gerencia
-- Fecha: 2026-05-28
-- DGG-XX · Extensión de #147 (Perfil Gestor):
--   A) Bucket 'gestor-uploads' público (lectura anon) con escritura anon
--      limitada a paths que empiezan con el token vigente.
--   B) Trigger tracking_linea_on_insert: cuando categoría='gestor_avance'
--      emite notif_emitir_staff (alerta a la gerencia).
-- ============================================================================

INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES ('gestor-uploads', 'gestor-uploads', true, 20 * 1024 * 1024, NULL)
ON CONFLICT (id) DO UPDATE SET
  public = EXCLUDED.public,
  file_size_limit = EXCLUDED.file_size_limit;

DROP POLICY IF EXISTS gestor_up_anon_select ON storage.objects;
CREATE POLICY gestor_up_anon_select ON storage.objects
  FOR SELECT TO anon, authenticated
  USING (bucket_id = 'gestor-uploads');
...
CREATE POLICY gestor_up_anon_insert ON storage.objects
  ...
  WITH CHECK (
    ...
    AND EXISTS (
      ...
      WHERE token = split_part(objects.name, '/', 1)
...
DROP POLICY IF EXISTS gestor_up_staff_all ON storage.objects;
CREATE POLICY gestor_up_staff_all ON storage.objects
  FOR ALL TO authenticated
  USING (bucket_id = 'gestor-uploads' AND private.is_staff())
  WITH CHECK (bucket_id = 'gestor-uploads' AND private.is_staff());

-- Trigger con bloque adicional de alerta a gerencia cuando categoría='gestor_avance'.
...
BEGIN
  ...
  -- Alerta a gerencia cuando viene del Panel de Gestor externo
