-- 0446_dgg142e4_f5_banner_alineado_matriz.sql
-- DGG-142 · Etapa 4 · F5 — `cliente_portal_dashboard` v3: banners del portal
-- alineados a la matriz DGG-143 y al motor diario (mig 0445). Base: definición
-- instalada por 0440 (verificada viva). Deltas:
--   (1) DDJJ "suave": el empujón nov→mar (antes sólo diciembre).
--   (2) `curso_actualizacion`: señal real = curso PBA de actualización en 12
--       MESES MÓVILES (antes ILIKE ambas jurisdicciones + año calendario —
--       deuda señalada en el plano E4).
--   (3) Consultoría jurídica visible TAMBIÉN para SOLO-CABA (decisión F: hoy
--       un SOLO-CABA jamás la veía porque gateaba por v_matriculado).
--   (4) CARD FORZADA el día del toque: si el motor tocó hoy a este cliente
--       (ofrecimientos_log canal 'banner', ancla = hoy ART), la card del
--       servicio aparece aunque la recurrencia del banner la hubiera
--       espaciado — respetando SIEMPRE el "posponer" del cliente
--       (snoozed_until gana).
--   (5) Gate de OPT-OUT: ofrecimientos_habilitados = false silencia todas
--       las cards de ofrecimiento (las de obligación dura — ddjj_proxima por
--       fecha, matrícula vencida/por vencer — se mantienen: son avisos de
--       estado, no marketing).
-- Firma intacta (0 args) → CREATE OR REPLACE conserva GRANTs (R16 no aplica).
