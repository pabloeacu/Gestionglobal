-- 0361 · JL-W8-3 (cont.) · fz_listar_movimientos expone identificado_at.
-- Habilita en la UI: chip "Identificado el <fecha>" (historial visible que pide
-- JL) y el botón "Deshacer identificación" SOLO en movimientos que pasaron por
-- el flujo (identificado_at NOT NULL). Extender el RETURNS TABLE exige
-- DROP+CREATE (42P13); los args de entrada no cambian → sin overload (R16),
-- pero re-emitir GRANTs.
...
CREATE FUNCTION public.fz_listar_movimientos(
  ...
  id uuid, caja_id uuid, caja_nombre text, caja_color text, fecha date,
  tipo text, monto numeric, categoria_id uuid, categoria_nombre text,
  descripcion text, referencia text, administracion_id uuid,
  administracion_nombre text, estado text, origen text,
  revertido_at timestamptz, transferencia_pair_id uuid,
  movimiento_revertido_id uuid, adjuntos_count bigint, total_count bigint,
  identificado_at timestamptz  -- JL-W8-3 · nueva, AL FINAL
)
...
AS $$
  ...
  SELECT
    ...
    adjuntos_count, total_count, identificado_at
  FROM with_count
...
REVOKE ALL ON FUNCTION public.fz_listar_movimientos(uuid, text, date, date, text, boolean, boolean, integer, integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.fz_listar_movimientos(uuid, text, date, date, text, boolean, boolean, integer, integer) TO authenticated, service_role;
