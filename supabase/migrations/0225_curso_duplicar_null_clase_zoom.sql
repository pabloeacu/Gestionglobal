-- §6 audit fixes para curso_duplicar:
--  · #C3: las clases tipo 'sincronica_zoom' copiaban zoom_url/zoom_fecha_hora apuntando
--    a la sala del curso ORIGINAL (incoherente con el NULL de curso_encuentros). Se NULLean.
--    youtube_url/material_url (contenido) SÍ se preservan.
--  · #A26: re-emite GRANT/REVOKE para que la migración sea idempotente (self-contained).
--  · #C7: comentario de mantenimiento sobre la enumeración explícita de columnas.
-- Misma firma → CREATE OR REPLACE (R16, sin overload).
...
BEGIN
  ...
  -- 9) encuentros (remap condicion_id; NULL salas Zoom/Webex → se crean de nuevo. WHERE de scope incluido)
