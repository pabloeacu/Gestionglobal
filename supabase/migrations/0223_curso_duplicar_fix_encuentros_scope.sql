-- FIX (cazado por e2e sintético, antes de shippear): el INSERT de curso_encuentros
-- en curso_duplicar (0222) clonaba TODOS los encuentros de la tabla por faltarle el
-- WHERE o.curso_id = p_curso_id (el LEFT JOIN al mapa de condiciones no filtra).
-- Re-creo la función con el filtro. Misma firma → CREATE OR REPLACE (R16: no overload).
...
BEGIN
  ...
  --    FIX: WHERE o.curso_id = p_curso_id (sin él se clonaban encuentros de otros cursos).
