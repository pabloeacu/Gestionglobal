-- 0320 · cliente_webinar_inscribirme: idempotencia REAL por email (auditoría §6,
-- agente C #4). La RPC chequeaba "ya inscripto" por administracion_id, pero la
-- clave única es (webinar_id, email_snapshot). Si la persona ya figuraba como
-- PROSPECTO (inscripta por el form público con el mismo email), el chequeo por
-- administracion_id no la encontraba → el INSERT chocaba webinar_inscriptos_unique_email
-- (23505). La UI lo humanizaba pero con un mensaje confuso ("ya existe un registro")
-- para alguien que "nunca se inscribió". Fix: ON CONFLICT (webinar_id, email_snapshot)
-- DO UPDATE → adopta la fila del prospecto (le fija administracion_id + profile_id)
-- y la devuelve. Idempotencia de verdad + cubre la carrera de doble submit. R16:
-- misma firma. Se preserva el search_path original (incluye 'auth').
...
BEGIN
  ...
  -- Ya inscripto bajo este cliente → idempotente.
  ...
  -- La clave única real es (webinar_id, email_snapshot). Si la persona ya figura
  -- (típico: prospecto por el form público con el mismo email), la ADOPTAMOS en
  -- vez de chocar 23505 → fija administracion_id + profile_id y devuelve esa fila.
  ...
  ON CONFLICT (webinar_id, email_snapshot) DO UPDATE
    SET administracion_id = EXCLUDED.administracion_id,
        ...
        prospecto_id     = NULL   -- al adoptar un prospecto, respetar el XOR de identidad
