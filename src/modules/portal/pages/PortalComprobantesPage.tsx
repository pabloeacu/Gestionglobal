...
// explícitamente por la administración del usuario (la RLS también filtra,
// pero ser explícito ayuda al planner y al UI).
...
export function PortalComprobantesPage() {
  const { user } = useAuth();
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <header>
        <p className="kicker text-brand-cyan">Mis comprobantes</p>
        <h1 className="font-display text-3xl font-bold text-brand-ink sm:text-4xl">
          Comprobantes
        </h1>
        <p className="mt-1 text-sm text-brand-muted">
          Listado de comprobantes emitidos a tu administración. Hacé click en
          uno para ver el detalle o descargar el PDF.
        </p>
      </header>
        ...
        <KpiCard
          ...
          value={
            <span className="tabular">
              $<AnimatedNumber value={Math.round(kpis.totalPagado)} />
            </span>
          }
          hint="cobrados"
          tone="teal"
        ...
        <div className="relative">
          {loading ? (
            ...
            <IllustratedEmpty
              ...
              description={
                <>
                  Ajustá los filtros para ver otro periodo, o esperá a que se
                  emitan nuevos comprobantes para tu administración.
                </>
              }
            />
          ) : (
