...
- **Decisiones de Pablo (cierre 2026-07-31, mig 0399):** (a) precio unificado $175.000 para PF y PJ — confirmado; (b) MATRÍCULA/LEGAJO RPAC al bloque principal de la máscara del gestor en ambas ramas (renovación/certificado los traen; en inscripción no existen y la máscara los omite sola); (c) columna `administraciones.cuit_titular_arca` + backfill automático en los 2 triggers + campo en la ficha (bloque RPAC) y en el drawer de edición — NO va en la grilla; (d) distinción gráfica en la grilla de clientes: ícono edificio SOLO para personas jurídicas (CUIT 30/33/34, helper `esCuitJuridico`), ícono persona para físicas.
