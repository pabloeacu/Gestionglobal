...
## DGG-121 · Sección "Mis certificados" permanente en el campus del alumno (2026-07-29)
- **Origen:** consulta de Pablo sobre cierre de trámites e historial del cliente. La investigación reveló el gap: al vencer la ventana de gracia (DGG-82) el curso desaparece de "Mis cursos" y el bloqueo de acceso tapaba también el botón del certificado — el alumno perdía el camino self-service para re-descargar su diploma (la RLS siempre lo permitió; el bloqueo era 100% de UI). Pablo eligió esta opción (#2) entre las sugerencias por ser la de menor riesgo que cierra el gap de raíz.
- **Diseño:** sección al pie de MisCursosPage que lista los certificados de CURSO propios (`listMisCertificados`: RLS por dueño + filtro explícito `alumno_profile_id = uid` como defensa en profundidad frente a la policy amplia de staff) con descarga por el pipeline probado del CertCelebracionBanner (esquema y título desde los snapshots congelados al emitir → cero dependencia del curso, que puede estar oculto). Los certs de evento quedan fuera (tienen su superficie en la ficha del evento). Sin migraciones: 100% frontend + service read-only.
- **§6:** auditoría adversarial OK (fuga por rol refutada: ruta gateada + filtro por uid + e2e RLS dueño=1/otro=0; eventos/prospectos excluidos por CHECK + uid; pipeline degrada a toast sin crash; nota 0 se muestra). Duda pre-existente documentada: un cert sin `esquema_snapshot` no sería descargable por el alumno (hoy 0 casos; la emisión lo persiste siempre desde mig 0087).

## DGG-122 · Gestoría Fase 1: asuntos, máscara de orden curado y "Ocultar a gestoría" (2026-07-31)
- **Origen:** docx de JL/Pablo con 4 consignas sobre lo que se envía/expone al gestor externo. Mandato: nada se borra ni se altera en el guardado — solo exposición.
- **Asuntos (migs 0395/0396):** los 2 mails a gestoría (derivación + re-aviso) pasan a "{{solicitante_nombre}}: {{servicio}}" (ej. "Catelli Federico Daniel: Inscripción al RPAC"). Hallazgo clave: dispatch-emails SIEMPRE re-renderiza el asunto desde email_templates (ignora email_queue.subject) → el cambio real fue de plantilla; el re-aviso ganó las vars que le faltaban y ambas RPCs encolan además el subject renderizado para la UI de la cola.
- **Máscara de orden curado (`mascaraGestoria.ts`):** causa del "orden aleatorio" que reportó JL = Postgres reordena las claves del jsonb. Una única máscara alimenta panel del gestor Y PDF: Personas Físicas (bloque principal 2 col + sub-sección antecedentes + nota Clave ARCA "no se pasa al RPAC") / Personas Jurídicas (su layout, detectada por razon_social) / "Otros datos del trámite" para TODO lo no mapeado (nada se suprime). Campos concatenados (nombre+apellido, calle+numero+piso+depto), fechas ISO → es-AR, etiquetas curadas con tildes.
- **visible_gestoria (mig 0395 + edge gestor-firmar-adjunto v5):** columna default true en formulario_adjuntos Y tramite_pedidos_doc_items + RPC staff-only `gestoria_set_adjunto_visible` + filtro en las TRES salidas (RPC del panel, var documentos del mail, firma de URLs — sin la 3ª sería cosmético). Toggles en wizard paso 2 (✗ Incorrecto PRE-OCULTA automático, destildable; ✓ restaura solo si fue auto; jamás bloquea Siguiente), ficha de solicitud y panel de pedidos de gerencia. Gerencia SIEMPRE ve todo.
- **§6 (3 auditores adversariales + e2e BD con rollback + BD viva):** hallazgo CRÍTICO cerrado — el estado del toggle vivía en el paso del wizard y se perdía al navegar (init stale podía mentir "Visible" con la BD en oculto, o pisar un ocultamiento manual): ahora vive en el estado del wizard + re-sync FRESCO desde BD al montar el paso. Menores cerrados: REVOKE de PUBLIC en la RPC (CREATE FUNCTION regala EXECUTE a PUBLIC — patrón a repetir), subject crudo en la cola (0396), 'departamento' del form PJ integrado al domicilio, overrides de etiquetas, guards anti doble-click, sync realtime del toggle de pedidos, CSS muerto del PDF. Residual aceptado y documentado: URLs YA firmadas antes de ocultar (PDFs viejos) viven su TTL de 1h; el escape HTML del asunto en dispatch-emails ('&' → '&amp;') es pre-existente y quedó anotado.
- **Verificado e2e:** derivación con adjunto oculto → var documentos lo omite; panel (token real) lo omite; asunto renderiza "QA E2E Gestoria: Inscripción al RPAC"; toggle staff ok / no-staff y anon 42501; R16 0 overloads; 0 residuos.

## DGG-123 · Switch Persona Física / Persona Jurídica en formularios RPAC (2026-07-31)
- **Un único form de inscripción dinámico** (mig 0397, snapshot previo de los 4 schemas en `formulario_versiones`): radio `tipo_persona_solicitante` ("Persona física"/"Persona jurídica") al inicio de matriculación, renovación y certificado. El form viejo `matriculacion-rpac-juridica` (roto, 0 submissions) se cosechó (hints de razón social/CUIT) y quedó DESACTIVADO junto con su servicio `rpac_inscripcion_juridica` — nada se borró.
- **Doctrina de identidad (capítulo delicado, palabras de Pablo):** el cliente ES la persona jurídica. CUIT y email de la EMPRESA viajan en las claves canónicas `cuit`/`email` (validan cuenta, dedupe, portal, facturación); razón social en `razon_social` (gana como `solicitante_nombre` en toda la cadena); el CUIT personal del titular en `cuit_titular_arca` — imprescindible para gestoría: a ARCA se entra con CUIT del titular + su clave fiscal. "Los mails podrían coincidir [titular vs sociedad]… los CUITs nunca" → **veto por CUIT**: `solicitud_match_cliente` no sugiere un cliente con CUIT distinto aunque coincida email/DNI (0397), y el bloqueo duro de `solicitud_activar` distingue el caso con un mensaje específico que pide un email propio para la empresa — el email sigue único porque es el login del portal (0398, hallazgo B#1 de la auditoría).
- **Formulario PJ (lista definitiva de Pablo):** razón social; datos personales = titular vinculado en ARCA (nota naranja aclara qué es de quién); domicilio compartido = sede social (nota) en inscripción y campo único `domicilio_empresa` en renovación/certificado; docs societarios: Constancia CUIT de la empresa, Constancia IIBB de la empresa, Certificado de vigencia (opcional, SOLO si no está inscripta en PBA), DNI de representantes técnicos en UN SOLO PDF (consigna "cada 20 edificios"); comparte planilla de consorcios, costos ($175.000 — ver flag abajo) y DDJJ. PF queda idéntico. Antecedentes personales (padre/madre/nacimiento/estado civil/cónyuge) y Formación profesional: solo-física.
- **Motor "sólo viaja lo visible" (el gap que el docstring prometía):** el runner ahora filtra datos+files por visibilidad al enviar, con CASCADA (dep oculto ⇒ valor vacío — sin esto el cónyuge quedaba visible al pasar de casado-PF a PJ) y las secciones sin campos visibles no se renderizan. El edge submit-formulario v12 replica el strip ANTES de la validación de identidad (defensa en profundidad, evaluación fresh sin memo = idéntica al runner). Restricción documentada: no nombrar extraDatos igual que un campo condicionado.
- **Downstream PJ (0398):** wizard prefillea IVA Responsable Inscripto + responsables desde nombre/apellido del titular (y `solicitud_activar` ahora los persiste); `domicilio_empresa` entra al armado de `direccion` en los 2 triggers de backfill (patrón E-GG-114); un cliente logueado que declara PJ NO backfillea el CUIT de la empresa sobre su propia cuenta (identidad por CUIT sagrada). Máscara de gestoría PJ: claves canónicas + TITULAR EN ARCA + DNI + PROVINCIA (hallazgo C#1) — mismo orden en panel y PDF.
- **Decisiones de Pablo (cierre 2026-07-31, mig 0399):** (a) precio unificado $175.000 para PF y PJ — confirmado; (b) MATRÍCULA/LEGAJO RPAC al bloque principal de la máscara del gestor en ambas ramas (renovación/certificado los traen; en inscripción no existen y la máscara los omite sola); (c) columna `administraciones.cuit_titular_arca` + backfill automático en los 2 triggers + campo en la ficha (bloque RPAC) y en el drawer de edición — NO va en la grilla; (d) distinción gráfica: ícono edificio SOLO para personas jurídicas (CUIT 30/33/34, helper `esCuitJuridico`), ícono persona para físicas — aplicado en grilla, avatar de la ficha y header del drawer (reactivo al CUIT tipeado); en Vencimientos·Por cliente el dataset no trae CUIT → ícono neutral Users. Cierre §6 (3 agentes): A y C sin GAPs de fondo; B#6 input del titular normalizado a dígitos (mismo formato que los triggers) y B#7 → mig 0400: el sync solo backfillea cuit_titular_arca sobre fichas jurídicas (e2e ambos sentidos con rollback).

## DGG-124 · Convención anti-grillas-infinitas: paginado + rango de fechas + encabezados que filtran/buscan/ordenan (2026-07-31)
- **Mandato Pablo (permanente):** ninguna grilla infinita; paginado default 50 con scroll cómodo; filtros de fecha "desde y hasta" en UN solo calendario; y los encabezados de toda grilla deben poder filtrar, buscar y ordenar. Todo SOLO FRONT y forma de traer para mostrar — cero migraciones, cero cambios de datos/lógica (aclaración expresa).
- **Toolkit reutilizable** en `src/components/common/`: `Paginador` (default 50, selector 25/50/100, variante sin-total con `hayMas` para RPCs sin count), `FiltroRangoFechas` (un calendario, primer click desde / segundo hasta, presets Hoy/7d/30d/Este mes, sin dependencias nuevas) y `ThInteractivo` (label ordena asc/desc; embudo abre popover de búsqueda de texto o checkboxes multi).
- **Página canónica: Cola de envíos** (EmailQueuePage): paginado server-side real (`.range()` + count), estado derivado traducido a filtros PostgREST exactos (dominio max_intentos {3,5} expandido por tramos — si aparece otro valor, agregar el tramo en emails.ts), rango de fechas sobre programado_para, orden por columna, filtro multi de plantillas, búsqueda saneada (%, _, comas y paréntesis rompían el .or/ilike) y **KPIs por counts head-only sobre el universo completo** (antes se calculaban sobre las 200 filas visibles → "Total" mentía; espíritu R19 sin bajar filas).
- **Aplicado además en:** Auditoría (la RPC siempre aceptó p_offset/p_desde/p_hasta y la página los ignoraba — era imposible ver más allá de 100 eventos; ahora pagina + rango de fechas), Errores runtime ("cargar 50 más" con tope 500 — la RPC no tiene offset), **Comprobantes** (paginado + KPIs de plata sobre el universo filtrado vía query liviana de 5 columnas con cap 5000 y filtros FACTORIZADOS con la grilla — antes sumaban solo 50 filas: cifras financieras incorrectas, violaba consistencia contable) y Campus (bajaba TODAS las matrículas históricas con 3 joins para 2 contadores → counts head + últimas 20).
- **Inventario completo** (agente §6): ~40 grillas clasificadas. Quedan para próximas tandas (front-only): Kanban de trámites sin banner de truncado, Prospectos, Moderación, Recupero, Partner portal, Portal comprobantes (cap 1000), Administraciones (KPIs sobre 50). **Requieren BD** (pedir autorización): ventana de fechas en `cliente_ctacte_extracto` (ledger all-time del portal — el más urgente), offset en `errores_listar`, RPCs de KPIs agregadas para reemplazar universos livianos. Código muerto detectado con paginación ya escrita y 0 consumidores: listTrackings, listVencimientos, listSubmissions, listMisTramites, listPagosReportadosCliente, getPortalDashboard.

## DGG-125 · ACL sweep: recorte de grants heredados muertos en anon/authenticated (2026-07-31)
- **Origen:** el default ACL del schema public regala los 7 privilegios a anon/authenticated en cada tabla nueva (capitalizado en DGG-120 con proveedores_frecuentes). Autorización de Pablo con cánones completos. **Regla de oro que hizo el riesgo ~cero: solo se revocó lo MUERTO** — privilegios sin policy RLS del comando para ese rol (RLS activa en las ~120 tablas, verificado) que nadie puede usar hoy. Matriz generada mecánicamente desde pg_policy + role_table_grants (mig 0401, 126 REVOKEs).
- **Criterios:** REFERENCES/TRIGGER/TRUNCATE revocados SIEMPRE (jamás usables desde el cliente; TRUNCATE ni siquiera respeta RLS); SELECT de authenticated conservado aunque no haya policy (0 filas ≠ error) salvo tramix_* (caches de edge, 0 usos en front); policies TO public cuentan para ambos roles; service_role intacto.
- **Resultado:** anon 11→4 grants (solo SELECT de formularios/servicios/categorias_servicio/servicio_vouchers, los públicos legítimos — pagos_reportados que tenía TODO quedó en cero) · authenticated 875→397 (0 de REFERENCES/TRIGGER/TRUNCATE, −98 writes muertos, −5 selects tramix). **485 permisos muertos eliminados.**
- **El refutador adversarial pagó el método:** cazó la ÚNICA línea que hubiera roto producción — push_subscriptions no podía perder UPDATE porque el front hace .upsert() y Postgres exige ese privilegio en parse-time aunque no haya conflicto (probado empíricamente). Y destapó E-GG-168: bug LATENTE pre-existente — sin policy UPDATE, la re-suscripción push con conflicto real (mismo user+endpoint con keys rotadas) fallaba en silencio desde siempre; 0401 agrega push_sub_owner_update (dueño) y el e2e confirmó el upsert con conflicto funcionando.
- **Verificación:** smoke R17 (0 triggers invoker escriben en tablas afectadas) · e2e con rollback (upsert push OK, INSERT a audit_log bloqueado 42501, anon lee formularios y NO pagos_reportados) · live QA (form público anon + gerencia QA: Inicio/Trámites/Clientes/Campus/Solicitudes sin un solo error de permisos, consola limpia) · limpieza QA a 0 · 100% reversible con GRANT. Pendiente señalado (no tocado, cambios de comportamiento): policies ALL:public de certificado_esquemas/vistas_guardadas/encuentro_sesiones_compartidas ameritan revisión de sus USING en otra tanda.

## DGG-126 · Switch PF/PJ preseleccionado en "Persona física" (pedido Pablo, 2026-08-05)

**Decisión:** el radio `tipo_persona_solicitante` de los 3 formularios RPAC (matriculación, renovación y certificado — el 3º sumado por consistencia: mismo motor, mismo riesgo) arranca preseleccionado en **"Persona física"** con su rama completa visible; el solicitante lo cambia si aplica. Antes arrancaba sin valor: ninguna rama visible, el solicitante completaba "a ciegas" los campos comunes y el freno del required recién aparecía al enviar — riesgo de presentaciones con información incorrecta, siendo PF la enorme mayoría. En el **portal**, el prefill del perfil pisa el default según la categoría del cliente: CUIT de FICHA 30/33/34 → "Persona jurídica" (doctrina DGG-123).

**Implementación:** (a) motor: `FormularioFieldDef.default` + el runner inicializa el estado desde el schema (solo si el default es una opción válida) — mig **0404** setea el default en los 3 schemas con snapshot; (b) portal: `fetchClientePerfilDatosFormulario` inyecta `tipo_persona_solicitante` derivado EXCLUSIVAMENTE de `_cuit_ficha` (clave meta nueva del RPC, mig **0405** — la clave `cuit` del dict cae a submissions históricas y no determina la categoría); (c) builder: Select "Valor inicial (opcional)" en radio/select (R14: la clave persistida tiene control de edición).

**§6 (workflow 7 agentes, 5 confirmados / 0 refutados) — cerrados en el mismo chunk:** (1) CRÍTICO: el prefill se re-ejecutaba en cada TOKEN_REFRESHED (deps por identidad del objeto `user`) y pisaba lo tipeado a mitad del llenado → deps `[user?.id]`; (2) CRÍTICO: el merge del prefill no respetaba `touched` → ahora jamás pisa un campo que el usuario tocó (el bug preexistía peor: el prefill REEMPLAZABA todo el state); (3) default huérfano si el builder renombra una opción → el runner lo descarta (y el edge ya 422aba "valor no permitido" como red); (4) builder sin control del default → Select agregado; (5) el preset podía basarse en un CUIT de submission histórica → `_cuit_ficha`.

## DGG-127 · Campos faltantes obligatorios en Inscripción y Renovación (pedido Pablo, 2026-08-05)

**Decisión:** Inscripción suma **Ciudad de nacimiento** (rama PF, junto a fecha de nacimiento/nacionalidad en "Datos personales"). Renovación suma **Fecha de nacimiento, Nacionalidad y Ciudad de nacimiento** (rama PF, en "Identificación" tras padre/madre) y la sección nueva **"Domicilio"**: Domicilio (rama PF — la PJ ya carga su sede en `domicilio_empresa`), Localidad y Código postal (comunes: para PJ completan la sede social). Todos required. Keys `direccion`/`localidad`/`codigo_postal` idénticas a las del RPC de perfil → **prefill automático en el portal**.

**Máscara del gestor:** DOMICILIO toma también `direccion`; **LUGAR DE NACIMIENTO** pasa a mostrar la ciudad real (`ciudad_nacimiento`; antes usaba la nacionalidad de fallback por no existir el dato) y **NACIONALIDAD** es campo propio del bloque "Información adicional". Históricos sin degradación (verificado en harness).
...
## DGG-131 · Encuentros sincrónicos compartidos RPAC ↔ RPA CABA: IA para Administradores + Asambleas Virtuales (pedido Pablo, 2026-08-07)

**Qué:** 10 fechas 2026 en los 2 módulos sincrónicos (modalidad `alternativos`: un solo presente cubre el requisito), **compartidas** entre "Curso de Actualización RPAC (Pcia. de Bs. As.)" y "Curso de Actualización RPA (CABA)" con la maquinaria F11/DGG-79 (sesiones compartidas — su caso fundacional eran exactamente estos 2 cursos). Todas 18:30-20:00 AR (90 min):
- **IA para Administradores** (Dra. Diana Sevitz): mié 26/08 (Zoom 82087050780), 23/09 (89353468140), 21/10 (81745026890), 25/11 (84201427368), 16/12 (83491238913).
- **Asambleas Virtuales** (Dr. Pablo Acuña): lun 31/08 (81085183995, **pre-existente** — se omitió crear, se ajustó 120→90 min y se compartió; Nota 2), 28/09 (82204551312), 26/10 (88564782922), 30/11 (89235512878), 28/12 (87280187681).

**Cómo (una por una, verificando cada paso — Nota 4):** INSERT del encuentro en RPAC (como gerente QA efímero vía PostgREST, RLS+triggers reales) → sala Zoom REAL vía edge `zoom-encuentro-create` (9 salas nuevas; la respuesta es la de la API de Zoom; join_url verificado vivo + unicidad global del meeting en BD) → `encuentro_compartir_con_curso` (la sala se MUEVE a la sesión compartida; el espejo CABA nace sin sala — imposible duplicar, UNIQUEs en ambas tablas) → **consolidación**: la RPC clona 1 condición-módulo por sesión (10 módulos hubieran roto la semántica `alternativos` exigiendo 10 presentes), así que cada espejo se reasignó a la condición canónica de su módulo y el clon sobrante se borró. CABA quedó con exactamente 2 módulos × 5 fechas, docentes con foto+CV clonados (Nota 1: no hizo falta crear módulos a mano — el clonado de la RPC los trae).

**Verificación §6:** sweep integral (10 sesiones con sala única, 0 meetings duplicados, 0 espejos desalineados, julio intacto y standalone, asistentes de julio siguen `cumplida`) + **e2e de fan-out con rollback forzado**: alumno sintético matriculado en ambos cursos entró a la sala de IA 26/08 (91 min) → `presente + umbral_cumplido` en el encuentro RPAC **y** en el espejo CABA + ambas condiciones `cumplida` — la semántica "un presente cubre ambos cursos" probada sobre las sesiones reales. Live QA gerencia: ambos cursos con sus 10 sellos "Compartido", leyenda "Misma sala que…", 0 botones "Crear sala Zoom" en espejos.

**Notas operativas:** (a) los 19 alumnos RPAC recibirán el mail recordatorio de las 8:00 AM y el banner "HOY tenés clase" cada fecha (comportamiento estándar del campus); (b) **gap conocido pre-existente**: la reconciliación post-reunión de Zoom NO cubre encuentros compartidos (mig 0371 los excluye) — la asistencia se computa en vivo vía el SDK embebido + pase de lista manual; monitorear la primera fecha (26/08); (c) en el portal Zoom la sala del 31/08 muestra duración 120 (cosmético — el meeting no se corta solo; en BD/campus manda 90); (d) CABA aún sin matrículas ni condiciones de pago/examen/encuesta — fuera del alcance pedido.

## DGG-132 · Generación CJ: edición visible, logo confiable, cortes de página limpios y numeración (pedido Pablo, 2026-08-07)

**Los 4 pedidos:** (1) **lápiz "Editar documento"** en Acciones del historial (la edición existía pero solo abría clickeando el tema — invisible); (2) **logo roto en el PDF**: causa raíz = el service worker de la PWA cachea `/logo-color.png` sin headers CORS y la re-carga `crossorigin` de html2canvas fallaba → hueco en blanco; ahora el logo viaja como **data URL** (fetch same-origin cacheado) idéntico en preview y PDF; (3) **cortes de página inteligentes**: el canvas se corta buscando hacia arriba la fila totalmente blanca más cercana (ventana 25% de página, piso 50%) — nunca más una línea de texto partida al medio; la página siguiente arranca en contenido (se saltea el colchón blanco); (4) **"N de M"** centrado al pie de cada página (helvetica 8pt, gris slate-300 clarito, banda reservada de 6mm).

**§6 (workflow 14 agentes + refutación, 8 confirmados / 4 descartados — todos cerrados en el chunk):**
1. (media) El data URL del logo podía quedar **envenenado para toda la sesión**: si el asset faltara, el rewrite SPA de Vercel devuelve index.html con 200 y el cache guardaba `data:text/html` → guard de `blob.type image/*` que degrada a la URL absoluta y resetea el cache.
2. (media) El `<style>` del layout se inyectaba en el documento VIVO durante la captura y **re-estilizaba toda la app** 1-3 s (la `.kicker` del PDF pisaba la clase global de labels) → todos los selectores scopeados bajo `.page`.
3. (media) **PDFs huérfanos en storage**: regenerar acumulaba `${Date.now()}.pdf` y eliminar el doc no borraba ninguno → path fijo `${docId}/documento.pdf` con upsert + `eliminarCjDocumento` barre la carpeta antes del RPC.
4. (media) **`cj-enviar-pdf` sin gate**: cualquier request con el anon key podía disparar envíos de PDFs jurídicos → v6 valida el JWT del caller y exige rol gerente (patrón zoom-encuentro-create; verify_jwt=false por el preflight CORS, auth interna).
5. (menor) iOS Safari limita el área del canvas (~16.7M px) y un doc de 5+ páginas salía en blanco sin error → cap dinámico de escala + guard de canvas vacío con mensaje humano.
6. (menor) Editar + solo "Guardar" invalida el PDF en silencio → nota ámbar en el footer del drawer («Guardar solo actualiza el texto — usá Generar PDF»).
...
## DGG-134 · Número de legajo en Actualización PBA + prefill/captura de datos constantes ("la plataforma ya sabe quién sos") (pedido Pablo, 2026-08-13)

**Pedido:** campo obligatorio "Número de legajo" bajo la matrícula en el form del Curso de Actualización RPAC (Pcia. de Bs. As.); y que los datos "constantes" (apellido, nombre, DNI, CUIT, matrícula, legajo…) vengan **auto-rellenos** (editables) al inscribirse desde el portal, capturándose a la ficha la primera vez que se declaran para reutilizarse después.

**Lo que ya existía y se reusó (0 componentes nuevos):** prefill del portal (`?origen=portal` + RPC `cliente_perfil_datos_formulario` + matching por `name` en FormularioRunner con sello "de tu perfil" y aviso "Pre-rellenamos N campos"); ficha con `administraciones.legajo_rpac` (mig 0167); dos triggers de captura con COALESCE aditivo (migs 0312/0340) que ya leían `datos->>'legajo_rpac'`.

**El hueco real (hipótesis remota confirmada con datos):** las inscripciones a cursos desde la landing llegaban sin `administracion_id` (30/31 reales) y el sync de submissions nunca corría para cursos; la ficha se salvaba solo por el trigger del trámite (0312). **DGG-134 cierra la vinculación**: al activarse la solicitud, la submission queda ligada al cliente (trigger con fallback por el trámite — mig 0420) y el sync corre en la transición NULL→valor.

**Implementación (migs 0419-0420 + 2 páginas):** campo `legajo_rpac` en el schema PBA (el `name` matchea la clave del prefill Y la de los triggers → todo se enchufa solo; CABA intacto); prefill v2 ('apellido'/'nombre' desde la ficha — los forms de cursos los separan y el full_name en 'nombre' quedaba mal; 'dni' prefiere `responsable_dni`; alias `numero_legajo`); backfill de 52 submissions históricas vinculadas (**delta real en fichas: 2 matrículas + 3 DNI** — el resto ya estaba capturado por 0312 en cada activación; hay DOS paths de captura redundantes a propósito, no desincronizarlos); eventos: el redirect `/formulario/eventos→/eventos` ahora preserva el query y `WebinarInscripcionPublicaPage` ganó el mismo prefill (antes el cliente logueado tipeaba todo).

**§6 (workflow 10 agentes + refutación, 4 confirmados / 4 cerrados + 4 descartados):** el hallazgo grande fue contra mí — mi trigger era **código muerto en el flujo real** (`solicitud_activar` marca 'procesado' ANTES de escribir `cliente_id`) y mi e2e dio **falso verde por simular el orden a mano** (lección E-GG-42 reincidente). Fix 0420 + smoke R18 ejecutando `solicitud_activar` REAL: submission vinculada + ficha capturada en la misma transacción ✓. Pendiente de gerencia (hallazgo de datos): **Biancofiore — ficha matrícula 2598 vs 109687 declarada en su inscripción; revisar contra el RPAC**.

**Verificación:** live en producción — landing anónima muestra "Número de legajo*" bajo la matrícula (schema-driven, sin deploy); portal logueado con ficha QA → **"Pre-rellenamos 8 campos con tus datos"** (los 8 de Identificación, editables); redirect de eventos preserva `?origen=portal` (verificado post-deploy). E2e rollback: captura vía RPC real, nunca-pisa, prefill completo. R16/R18 OK. Limpieza QA a 0.

## DGG-135 · Adjuntos opcionales en "Avisar a la gestoría" — viajan como archivos reales en el email (pedido Pablo, 2026-08-13)

**Pedido:** dentro de la ventana de "Avisar a la gestoría" (que DGG-133 dotó de mensaje libre), poder adjuntar archivos opcionales.

**Diseño:** modal nuevo `AvisarGestoriaModal` (reemplaza el `usePrompt` de DGG-133) que reusa el circuito de la derivación original completo: staff sube a `gestoria-adjuntos` (`uploadAdjuntoGestoria`, prefijo `solicitud_id/`), la RPC `derivacion_reavisar_gestoria` (mig 0421, +`p_adjuntos jsonb` — R16: DROP de la firma vieja) pasa los metas `{path, filename, mime, size}` a `email_queue.attachments_jsonb` y el despachador (DGG-118) adjunta los ARCHIVOS REALES al mail. Subida al ENVIAR (no al elegir — anti-huérfanos E-GG-177) con cache incremental de metas (un retry no re-sube ni duplica). La línea de tracking sigue SIEMPRE interna (`visible_cliente=false`).

**§6 (workflow 11 agentes: 2 dimensiones + refutación adversarial; 6 hallazgos únicos confirmados, todos cerrados en el chunk — mig 0422 + modal):**
1. **(crítica)** El techo real de adjuntos del mail es `MAX_ATTACH_B64_TOTAL` = 7M chars b64 ≈ **5,25 MB TOTALES** y el despachador **omite en silencio** el excedente (el mail sale 'sent' igual; `gestor-firmar-adjunto` no cubre este bucket → el email es el ÚNICO canal de entrega) — mi modal prometía 10×20 MB: pérdida silenciosa clase E-GG-177 con toast y tracking mintiendo. Fix: espejo honesto **4,5 MB en total** en el modal (por archivo + total acumulado + guard en submit, copy actualizado) **+ RAISE server-side** si `SUM(size)` supera el techo (mig 0422).
2. (media) Los adjuntos del reaviso no aparecían en el listado E-GG-90 del trámite (`tramite_docs_cliente` lee `solicitud_derivaciones.adjuntos_jsonb`, no el folder del bucket) → la RPC los anexa ahí con dedupe por path (reintentos no duplican).
3. (media) El modal se podía cerrar con ESC/backdrop/X con el envío en vuelo → `safeClose` gated por `enviando`.
4. (menor) `variables.adjuntos_count` era data muerta (la plantilla no renderiza variables nuevas; renderVars es sustitución plana) → la mención "Te adjuntamos N archivo(s) en este email." va ANEXADA a `mensaje`, que la plantilla sí renderiza con pre-wrap.
5. (menor) Cache anti-re-subida roto si fallaba a mitad de lote → cacheo incremental por archivo subido.
6. (menor) Subidos-nunca-enviados quedaban huérfanos sin detector (el cron E-GG-177 cubre otro bucket) → limpieza best-effort al cerrar sin enviar y al quitar una fila ya subida.
(+) Hardening derivado de un descartado: la RPC re-construye cada elemento con SOLO las 4 claves del contrato — claves extra del JSON entrante (`content_b64`, `storage_bucket`) jamás llegan a `attachments_jsonb`.

**Verificación:** e2e BD rollback forzado T1-T5 ejecutando la RPC real (saneo de claves + mime default, dedupe 3-no-5, techo rechaza 5 MB con 22023, sin-adjuntos idéntico a antes, `tramite_docs_cliente` lista los 3) + smoke R16 (0 overloads). Live en producción (vbe67ed7, gerente QA efímero + trámite/derivación QA con destinataria QA — jamás la gestoría real): modal con copy honesto, rechazo visible del archivo de 5 MB («grande3.pdf pesa 5.0 MB y el máximo es 4.5 MB»), envío real con 2 archivos sintéticos → toast "Con 2 archivos adjuntos", mail en cola con `attachments_jsonb(2)` saneado + mención en el mensaje (neutralizado con UPDATE-solo ANTES de todo SELECT — canon qa-dgg133), línea interna correcta, tab Documentación lista ambos "Enviado a gestoría", storage con los 2 archivos reales; mobile 360px impecable (capturas). Los 2 errores 400 de consola son artefacto del método de adopción de sesión QA (refresh token single-use), no del producto. Limpieza a 0 verificada en 10 superficies (auth, profiles, trámite, solicitud, derivación, líneas, colas, sent_emails, accesos, storage).

## DGG-137 · El sello "Compartido" del alumno solo cuando le es relevante (pedido Pablo, 2026-08-13)

**Pedido:** el sello "COMPARTIDO" + "tu presente cuenta en ambos cursos" en los encuentros sincrónicos del campus despistaba a alumnos matriculados en UN solo curso (la mayoría). Regla nueva: gerencia lo ve siempre; el alumno solo si él mismo tiene matrícula ACTIVA en 2+ cursos que comparten esa sesión.

**Implementación (frontend puro, sin mig):** `listSesionesCompartidasRelevantes` en campus.ts — filtrado EXPLÍCITO por los otros cursos activos del propio alumno (no depende de RLS): 2 queries livianas que corren solo si el curso tiene sesiones compartidas. `CursoDetalleAlumnoPage` calcula el set y lo pasa a `EncuentrosEnVivoAlumno`; el sello y la leyenda quedan gateados por `sesionesCompartidasRelevantes.has(sesion_compartida_id)`. `EncuentrosTab` (gerencia) intacto. Copy nuevo: "tu presente cuenta en tus dos cursos".

**Verificación:** e2e BD con rollback forzado bajo el RLS real del alumno QA (1 matrícula → 0 relevantes; 2 matrículas → 10 sesiones relevantes — las 10 compartidas RPAC↔CABA de DGG-131; sanity: sin matrícula el alumno ni ve los encuentros del co-curso, RLS DGG-115). Live en producción (da63533): alumno QA de 1 curso SIN sello ✓; con 2ª matrícula temporal el sello y la leyenda aparecen ✓; matrícula temporal eliminada (limpieza verificada, 1 sola matrícula QA restante — fixture del tema DGG-136).

## DGG-136 · Nueva dirección visual "marca-nativa" (tema gg-brand) — de kit externo a CUTOVER en producción (pedido Pablo, 2026-08-13/14)

**Pedido:** aplicar el UI kit `gestionglobal-ui-kit/` (Oswald+Archivo, navy/petróleo/cyan en capas, esquinas biseladas, motivo triangular) a TODA la plataforma como tema paralelo conmutable y reversible; "solo maquillaje" — cero lógica. Leyes de Pablo: (1) PDFs intocables (estética y lógica de render), (2) alcance total pantalla por pantalla, (3) landing pública idéntica. Modelo de flag acordado: por dispositivo durante la prueba + cutover global por deploy. Cierre: "dejá el nuevo tema en ON... es mejor apagarlo si falla".

**Arquitectura (la clave del riesgo <2%):** paleta `brand.*` de Tailwind convertida a `rgb(var(--brand-*-rgb)/<alpha>)` con `:root` = hex históricos exactos (pixel-perfect verificado con flag off) → las ~4.700 utilities compiladas se re-tematizan redefiniendo 12 variables bajo `[data-theme='gg-brand']`. Overrides 100% scopeados a superficies hookeadas (`[data-gg]` — atributos inertes en PortalLayout/GerenciaLayout/páginas; los overlays porteados llevan `data-gg="overlay"`). Flag: `public/theme-boot.js` pre-paint (CSP 'self', no-cache) con `localStorage gg.ui.theme.v2` + atajos `?tema=gg-brand`/`?tema=clasico`; DEFAULT en el propio archivo. Fuentes self-host (`GG Oswald`/`GG Archivo`, 4 woff2 variables, lazy).

**Los 3 escapes estructurales (probados empíricamente):**
- **PDFs**: los 5 pipelines montan con `data-gg-classic` (reset de las 12 vars + font). Prueba reina con tema ON y fuentes cargadas: stack del certificado = Impact exacto (163.31px vs 167.86 de GG Oswald), colores/radius/sombras clásicos dentro del escape.
- **Impresos**: TODO el tema visual vive bajo `@media screen` — al imprimir el tema desaparece entero (verificado imprimiendo PDF con Chrome headless: la orden del gestor vuelve a Sora/redondeada).
- **Landing**: `data-gg-classic` en LandingPage/ComingSoonCoverPage — con tema ON, adentro miden los valores clásicos exactos (teal 27 157 168 vs 21 154 166 en root).

**Las 3 §6 (44 agentes, ~800 verificaciones):** Fase 0 cazó el CRÍTICO de que `@font-face 'Oswald'` capturaba el stack del título del certificado (Bebas→Oswald→Impact) cambiando el PDF — fix: familias `GG *` (reproducido y re-probado por medición de píxeles). La integral (30 agentes) cazó 2 CRÍTICAS: `box-shadow:none` mataba el focus ring de a11y (fix ring-safe con canal compuesto) y los overlays porteados quedaban sin tema (fix data-gg="overlay" en Modal/Drawer/CommandPalette/NotificationBell/Tour/Toasts); + candado print incompleto, kanban sin affordance, BrandMark invisible en navy, badge campanita, key localStorage versionada, SW gg-v3, descriptor de fuente honesto 400-700, superficies híbridas (/restablecer,/plataforma,/health), y hardening del boot con storage bloqueado.

**Verificación integral:** barrido 54/54 rutas de gerencia (todas las estáticas + 8 detalles con IDs reales) con tema ON: Oswald aplicado, sin overflow, contenido completo; portal (9 rutas), campus alumno, login, partner, gestor externo, público — todos verificados en vivo con fixtures QA; mobile 360px spot-checks; DGG-137 (sello Compartido) resuelto en el medio y mergeado a ambos mundos.

**CUTOVER (36d2af5):** DEFAULT='gg-brand' — todos los roles ven la dirección marca-nativa. **ROLLBACK: cambiar DEFAULT='classic' en public/theme-boot.js + push (~2 min); por dispositivo: `?tema=clasico`.** El no-cache de theme-boot + CACHE_VERSION gg-v3 garantizan flip instantáneo (PWAs via reclaim E-GG-155). Tolerancia documentada: modificadores /alpha sin quantizar a hex (delta ≤0,14%). Pendientes de refinamiento estético (no funcionales): tonos de HotCards del portal, charts con paleta clásica (que ya es de marca), ribbon decorativo del mockup omitido. `src/lib/theme.ts` reservado para el futuro toggle de UI (espejo de theme-boot, regla anti-divergencia comentada en ambos).

### DGG-136 · Anexo post-cutover: disección milimétrica del kit (pedido Pablo, 2026-08-14)

Pablo, con el tema ya en producción: "los chips del modelo son rectangulares y todos los chips siguen ovalados... los vértices de las grillas; el estilo de los títulos... revisalo milímetro a milímetro" + "los modelos son de algunas pantallas, pero trasportá el estilo a todos los espacios de todas las secciones". Respuesta: **bloque D de gg-theme.css**, selectores GENÉRICOS bajo `[data-gg]` (portal, gerencia 54 rutas, campus, partner, externo, auth, público y 26 overlays a la vez): D.1 chips (badge estático → chamfer 6px + Oswald MAYÚSCULAS; pill interactiva → cuadrada sin clip para no recortar el focus ring — la semántica del kit casualmente mapea 1:1: estado=span=chamfer, filtro=button=cuadrado); D.2 títulos h2/h3.font-display → MAYÚSCULAS + regla bicolor petrol→navy; D.3 vértice plegado de .card-premium por background multicapa (cero clip-path/position → cero riesgo funcional); D.4 th Oswald sobre paper + hover pale; D.5 botones Oswald + offset duro hover; D.6 labels micro/inputs paper; D.7 kbd; D.8 progress degradé; D.9 tabs; D.10 KPI cards (data-gg-kpi por tono); D.11 rail excluido (Archivo).

**GAP cazado en la verificación de producción post-cutover** (el §6 integral hookeó solo los 6 overlays de common/): los 20 `createPortal` de módulos (PortalOnboardingTour, uploaders del campus, EncuentrosTab, agenda, conciliación, partner, solicitudes, trackings, trámites, webinars, popover de FormularioRunner) montaban en body FUERA de `[data-gg]` → renderizaban clásicos. Fix mecánico por regex: `data-gg="overlay"` en los 20. **Blindaje nuevo de certificados**: los 5 lienzos on-screen de CertificadoPremium/ConstanciaPremium quedaron envueltos en `data-gg-classic` — el preview en pantalla es por construcción lo mismo que el PDF (prueba reina re-corrida: stack→Impact exacto, teal clásico). Lección para ERRORES: "hookear overlays" debe barrer `grep createPortal src/` completo, no solo components/common.

### DGG-136 · Anexo II: bloque E — fidelidad total por auditoría de 6 lentes (2026-08-14)

Pablo (portal del alumno): "siguen habiendo diferencias: color y estilo de los botones, íconos lucide, vista preliminar del curso... no quiero volver a tener que revisar cosa por cosa". Respuesta: workflow de 6 agentes comparando CADA regla CSS de los dos mockups contra gg-theme.css + componentes reales → **97 gaps** (14 altas / 43 medias / 40 bajas), TODOS aplicados en el bloque E + 13 hooks TSX inertes. Piezas mayores: botones del kit completos (chamfer 8px apagado en :focus-visible, 12.5px, hover por drop-shadow porque el clip-path recorta box-shadow — hallazgo de la propia auditoría —, fondo estable, :active, rojo #C22B4A, halo de foco de la spec, ▶ data-gg-tri); rombo de estado en badges + paleta semántica del kit; eyebrows petrol globales; portada de curso SIEMPRE (fallback degradé+motivo+título vía data-gg-cover-fallback; con banner real el motivo NO se superpone — esa mezcla era la "vista preliminar" que señaló Pablo); c-hero sin banner; devices remind/notice/alarm (los banners amber → línea izquierda 3px del kit); empty states con mtile; progreso pale+cyan→petrol; nav de gerencia (grupos micro-label, flechitas, filete activo); topbar sólido (search paper 250px, icon-btns 38, saludo Oswald 19); lucide 2→1.8 con guard [stroke-width='2']; bordes slate→línea navy-tintada global. Verificado en producción con la sesión real de Pablo (portal Inicio + campus). Pendientes deliberados: quote dorada del banner de certificado (paleta de celebración, validar), íconos temáticos por sección en sec-head (motivo como sustituto), delta de KPIs (falta el dato), avatar docente sin foto (necesita iniciales).

### DGG-136 · Cierre §6 total del tema (auditoría de 3 frentes, 2026-08-14)

Pablo pidió cierre sin pendientes ("producción, clientes reales, plata real, JL con lupa"). Doble auditoría §6 con 3 agentes en paralelo (canon del proyecto) barriendo TODO antes de cerrar:
- **Funcional:** cazó E-GG-180 (borde rojo de validación anulado — regresión real) + confirmó por barrido que no había repetición del bug del topbar ni click-hijack por pseudos/clip-path; descartó 7 falsos-riesgos con evidencia. Fixes: exclusión de `border-red` en inputs, `:not([data-gg='overlay'] *)` en D.12 simple (paridad con D.12b), `pointer-events:none` en pseudos decorativos.
- **Fidelidad:** el widget descolocado eran SEIS (todos los banners "en vivo" del Inicio de gerencia doble-cajeaban: span-ícono propio + caja degradé del sec-head) → `data-gg-plain` en los 6. + íconos de sección faltantes (Sparkles/CalendarCheck/Video).
