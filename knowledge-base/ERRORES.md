...
### E-GG-151 · (DGG-116 · hallazgo de la §6) 2 edge functions re-implementaban el gate de publicación con la lógica vieja de 4 estados
- **Síntoma (latente):** al simplificar el modelo de publicación de cursos a 3 estados (DGG-116, mig 0380), un curso con **visibilidad anticipada** — `activo=true` (check "Visible" puesto) + `publicar_at` (Fecha de inicio) **futura** — deriva a `'publicado'` en BD: RLS expone el contenido, la clase aparece en dashboard/banner/campus y el alumno matriculado la ve. Pero al intentar **entrar al encuentro en vivo** (Zoom/Webex) recibiría **403 `curso_no_publicado`**. El alumno ve la clase en todas las superficies y no puede entrar → ruptura de la consistencia entre superficies.
- **Causa raíz:** `zoom-sdk-signature/index.ts` y `webex-guest-token/index.ts` **re-implementan** el gate de publicación en TypeScript (por performance, para no llamar al derivador) con la fórmula vieja `publicado = activo && (!publicar_at || publicar_at<=now)`. Eso espejaba el modelo de 4 estados (donde `publicar_at` futuro RETENÍA la visibilidad = 'programado'). DGG-116 eliminó esa retención pero los dos edges quedaron en la fórmula vieja. **El grep de cierre de la mig 0380 fue scopeado a consumidores SQL** ("ningún consumidor compara con 'borrador'/'programado'") y por eso NO los vio: los edges no referencian el nombre del derivador ni los literales de estado, así que escaparon a todo el barrido SQL. Reachability real al momento del fix = 0 (ningún curso vivo usa `publicar_at`), pero es exactamente el caso de uso estrella de la feature → bug latente clásico (lección E-GG-42: el bug que se manifiesta con el primer uso real).
- **Fix:** en ambos edges, `const publicado = !!curso?.activo;` (quitar la condición `publicar_at`), espejando el derivador de 3 estados. `finalizado` (despublicar_at<=now) se sigue calculando aparte → un matriculado conserva acceso post-fin (DGG-82) y un `no_visible` (activo=false) sigue recibiendo 403 (coherente con la RLS de las tablas hijas, que no expone contenido a cursos no visibles). Deployados: zoom-sdk-signature v11 (verify_jwt=true), webex-guest-token v6 (verify_jwt=false); config preservada.
- **Lección:** cuando un valor derivado en BD tiene ESPEJOS en otras runtimes (edge functions en TS, front), el smoke de cierre de una migración que cambia la derivación NO puede scopearse sólo a SQL — hay que grepear TODO el repo (edges + front) por la lógica re-implementada, no sólo por el nombre de la función o los literales. La doble §6 (Agente A) lo cazó justamente porque su foco incluía "objetos que re-implementan el estado", no sólo "objetos que llaman al derivador".

### E-GG-152 · (reporte de JL) La observación interna del wizard de activación se perdía silenciosamente ("Categoría inválida")
- **Síntoma:** al completar el wizard de activación de una solicitud, JL vio "observaciones" en 3 casos (D´Imperio 14:14, Trotta 14:20, PALOPOLO 14:31 del 23/07): dos rotuladas como el paso "Registra observación en el tracking" y una como "Categoría inválida". Los 3 son el MISMO problema, transcripto con distinto detalle.
- **Causa raíz:** el paso 1b del `ProcesadorFinal.tsx` (registrar la observación interna que la gerencia carga en el paso "Tracking") llamaba `agregarLinea(..., { categoria: 'alta', ... })` → `rpc('tracking_agregar_linea')`. La RPC valida `p_categoria` contra `tracking_categorias_config`, y **`'alta'` NO está en el catálogo** (17 slugs válidos: seguimiento_interno, gestor_avance, aprobacion, cierre, custom, etc. — ninguno 'alta'). La RPC rechaza con `RAISE EXCEPTION 'Categoría inválida: alta'` (ERRCODE 22023). Como el paso es `bestEffort: true`, NO frena el trámite — pero la observación que la gerencia escribió **se perdía sin aviso claro** (sólo el warning). Bug PRE-EXISTENTE, sin relación con DGG-116: `SELECT count(*) FROM tracking_lineas WHERE categoria='alta'` = **0 en toda la historia** → nunca funcionó desde que se escribió el paso. El resto del wizard (alta de cliente + apertura del trámite + matrícula del curso + línea de apertura automática `seguimiento_interno` visible) se completó correctamente en los 3 casos.
- **Fix:** `categoria: 'alta'` → `categoria: 'seguimiento_interno'` (misma categoría que usa la línea de apertura automática, acá con `visible_cliente=false` para que sea nota interna). Es la única aparición de `categoria: 'alta'` en todo el repo. Categoría verificada válida (las 3 líneas de apertura de hoy la usan con éxito).
- **Dato para operación:** las 3 observaciones que JL escribió NO quedaron guardadas (nunca llegaron a la BD). Hay que re-cargarlas a mano en cada trámite (D´Imperio `11fe0d59`, Trotta `b464fdec`, PALOPOLO `48eaa4e0`). El texto no es recuperable porque el INSERT fue rechazado antes de persistir.
- **Lección:** las categorías de tracking pasaron a ser un catálogo en BD (`tracking_categorias_config`), pero un caller en el front todavía usaba un slug legacy hardcodeado que nunca migró. Todo slug de categoría escrito a mano en el código debe existir en el catálogo — o mejor, exponer las categorías válidas como constante/type compartido para que el compilador lo cace.

### E-GG-153 · (reporte de JL) El paso Cobranza del wizard no permitía cargar fecha ni N° de transferencia/comprobante
- **Síntoma:** al cobrar en el wizard de activación (paso "Comprobante y cobranza"), JL no encontraba dónde registrar (a) la fecha real de la transferencia y (b) el N° de transferencia/comprobante. Terminaba anotándolos en el campo "Observaciones" del paso Tracking (ver E-GG-152), que además se perdía.
- **Causa raíz:** el paso Comprobante sólo capturaba modo de cobro, monto, caja, categoría y partner. En `ProcesadorFinal` la cobranza se registraba con `fecha: hoy()` (hardcodeada) y `referencia: ''` (vacía). Sin embargo, la RPC `registrar_cobranza_comprobante` YA aceptaba `p_fecha` y `p_referencia` y los persistía en `movimientos.fecha` / `movimientos.referencia` (con `NULLIF(trim(),'')`) — sólo que el wizard nunca los pasaba. NO era falta de modelo: era falta de captura en la UI.
- **Fix (100% front, sin migración):** se agregaron 2 campos al bloque Cobranza del wizard — **"Fecha del pago"** (date, required, default hoy, editable — puede diferir de la fecha del comprobante) y **"N° de comprobante / transferencia"** (text, opcional). `ComprobanteState` sumó `fechaPago` + `referencia`; `ProcesadorFinal` ahora pasa `fecha: c.fechaPago || hoy()` y `referencia: c.referencia?.trim() || ''`. La cobranza queda en `movimientos` con la fecha real y el número, visibles en el extracto/ficha del cliente y en el módulo de Finanzas.
- **Verificado:** e2e con datos reales (rollback) — `registrar_cobranza_comprobante(..., '2026-07-20', ..., '945418')` deja el movimiento con `fecha=2026-07-20` y `referencia=945418` (no `hoy()` ni vacío). Build limpio + prueba en vivo del wizard.
- **Lección:** cuando la RPC ya expone parámetros que el front descarta con valores hardcodeados, el gap es de UI, no de modelo — grepear qué parámetros de la RPC se pasan constantes revela features "a medio cablear".

### E-GG-154 · (reporte de Pablo) Los cards de cajas y KPIs de Finanzas redondeaban los centavos → cifra inexacta para conciliar
- **Síntoma:** el card "MP. Gestión Global" del dashboard de Finanzas mostraba `$ 3.074.548` cuando el saldo real en BD era `$ 3.074.548,26`. Para conciliación bancaria una cifra redondeada es una cifra incorrecta.
- **Causa raíz:** 5 formatters locales `formatMoney` con `maximumFractionDigits: 0` (copy-paste del mismo patrón) en `FinanzasDashboardPage`, `FinanzasAdminPage`, `FinanzasReportesPage`, `TransferenciaModal` e `ImportadorExtractoModal`. La tabla de movimientos usaba `formatMonto` (2 decimales) — la MISMA cifra se veía distinta según la superficie, violando el canon de consistencia contable.
- **Fix (100% front):** (a) los 5 formatters pasaron a `minimumFractionDigits: 2, maximumFractionDigits: 2` (exports PDF/XLS, selects, tablas y confirms ahora exactos); (b) en el dashboard se agregó `MoneySup` — renderiza los centavos en superíndice (`<sup>` a 0.62em, idea de Pablo) para no comerse el ancho del card — aplicado a los 4 KPIs y a los cards de "Cajas activas"; (c) `KpiCard.value` pasó de `string` a `ReactNode`. El branch `compact` de ReportesPage (ejes de gráficos, `$ 3,1 M`) se mantuvo: los ejes aproximan por diseño.
- **Adyacencias cazadas por la triple auditoría §6:** (1) `PasoGestoria.tsx` mostraba el MISMO saldo de caja con `toLocaleString('es-AR')` sin decimales fijos (0-3 decimales variables) → formato canónico `formatMoneyExact`; (2) los KpiCard del dashboard desbordaban 9px a 360px (medido en vivo con scrollWidth) → `text-base sm:text-xl`; (3) tablas Flujo/Comparativo de Reportes y preview del importador sin `overflow-x-auto`; (4) columna Monto del PDF export con width fijo 12% → 14%.
- **Verificado:** build limpio + live QA sobre el bundle final deployado (footer vb251435, gerente QA efímero, 0 residuos) comparando contra `SELECT saldo FROM cajas_con_saldo` (3.074.548,26 / 880.000,00 / 0) — cards y KPIs exactos al centavo con `<sup>`, desktop y mobile 360 sin desbordes (0 overflows medidos), consola limpia, tabla de movimientos intacta.
- **Lección:** un formatter de moneda con `maximumFractionDigits: 0` sólo es válido para cifras decorativas (KPIs de volumen, ejes). En cualquier superficie de conciliación (saldos de caja, extractos, reportes contables) es un bug de exactitud. Smell check: `grep "maximumFractionDigits: 0"` sobre módulos financieros.

### E-GG-155 · (reporte de Pablo y JL) Deslogueos recurrentes al retomar el trabajo — DOS causas, la dominante invisible para el server
- **Síntoma:** JL y Pablo quedaban deslogueados 1-2 veces/día, siempre al retomar la actividad (mañana, post-almuerzo). El audit_log mostraba re-logins forzados; las sesiones rotaban perfecto 14-24h cuando nadie las tocaba.
- **Causa raíz A (la dominante, cazada por el e2e en vivo · E-GG-155b):** al abrir la app con el access token vencido (pausa >1h), el boot refrescaba OK (200 del server) pero `INITIAL_SESSION` —que auth-js SIEMPRE emite con sesión null al suscribirse, porque usamos `persistSession:false`— pasaba por el bloque de persistencia del handler y **wipeaba `gg.auth.session`**; el guard anti-resurrección de E-GG-144 veía el storage vacío y **descartaba la sesión válida recién refrescada** → "se me cerró la sesión". Pre-existente desde E-GG-144, CERO rastro de error en el server (los re-logins "sin 400"). Fix: el handler solo toca el storage en `SIGNED_IN`/`SIGNED_OUT`/`TOKEN_REFRESHED`.
- **Causa raíz B (la del diagnóstico inicial):** pestañas/PWA "zombies" abiertas por días corren un bundle pre-E-GG-144 en RAM (sin lock cross-tab) → rotan el refresh token por afuera → GoTrue detecta reuso fuera de la ventana de 10s → revoca la FAMILIA → las pestañas sanas reciben 400 y deslogueaban al instante. El fallback offline del SW agravaba: PWA abriendo antes de que vuelva el WiFi recibía un index añejo con bundle pre-fix.
- **Fix (3 frentes + hallazgos §6):**
  1. **AuthContext**: `refreshInFlightRef` difiere el SIGNED_OUT que auth-js emite durante un refresh en vuelo (el storage llega intacto al path del 400); ante 400: adopción del token de otra pestaña → retry único por token (1.5s; boot 1.2s) → logout limpio solo al segundo 400. Sin loops (verificado contra auth-js 2.106).
  2. **Anti-zombie (SW gg-v2 + main.tsx)**: `reg.update()` al ganar visibilidad; `controllerchange` → auto-reload (inmediato si oculta, diferido al ocultarse si visible; nunca con Drawer/Modal abierto ni en /restablecer); el SW navega los clients ocultos de bundle viejo (ping/pong paralelo: los nuevos responden y se autogestionan). `updateViaCache:'none'` + Cache-Control explícito del SW en vercel.json. Nadie vuelve a quedar clavado en un bundle viejo.
  3. **loadProfile** (hipótesis alternativa del refutador §6): una RE-carga de perfil fallida (3 timeouts con red lenta post-suspensión, ~30s) ya no tumba una sesión operando — conserva el perfil en memoria y reintenta al próximo ciclo. Ese signOut también deslogueaba sin 400. Bonus: `accesoRevocado` se auto-limpia sin depender del handler (el catch prometía limpiar storage y no lo hacía).
- **Verificado (e2e en vivo sobre el bundle deployado, QA efímero, 0 residuos):** Test A access vencido + reload → **sesión rotada, sigue logueado** (el flujo matutino, antes moría); Test B dos pestañas en carrera de refresh → ambas vivas; Test C token corrupto → exactamente 2 intentos (retry 1.2s medido: 1129ms/2484ms) → logout limpio sin loop; Test 0 SW gg-v2 se instala y purga gg-v1 sin intervención manual.
- **Lección doble:** (1) el "ejercitar" del §6 vale oro: la causa A era invisible a TODA lectura estática (el server devolvía 200; el cliente tiraba la sesión buena) y solo apareció al probar en vivo "access vencido + reload" — un flujo que ningún QA había ejercitado porque los logins de prueba siempre nacen frescos. (2) Un handler de eventos de auth que persiste "cualquier evento" es una bomba: cada evento nuevo de la librería (INITIAL_SESSION) se convierte en un side-effect no diseñado. Gatear SIEMPRE por lista blanca de eventos.

### E-GG-156 · (reporte de Pablo) "Corregir mail de acceso": mail al correo muerto + avisito en vez de bienvenida + semáforo stale
- **Síntoma (captura de Pablo, caso Nogueira 24/07):** tras corregir el email de acceso, el sistema envió un aviso al correo VIEJO (el que justamente estaba muerto) y al correo nuevo le llegó un avisito sin credenciales, cuando la gerencia — viendo el semáforo amarillo — esperaba la bienvenida con contraseña.
- **Causas raíz (tres, una por capa):**
  1. **Diseño (edge):** el "aviso de seguridad al mail anterior" (§6 A#8 de DGG-117) copiaba un patrón estándar de cambio de credenciales sin pesar el negocio: acá el correo viejo está MUERTO por definición del caso de uso — escribirle solo genera otro rebote, ruido en el banner de rebotes y confusión.
  2. **Contenido (template):** el mail del caso "nunca ingresó" llevaba la contraseña pero como avisito, no como la bienvenida canónica completa (pedido explícito de Pablo: breve mención del cambio + todo el texto de bienvenida de siempre).
  3. **UI (forense):** el semáforo de la ficha estaba STALE — Nogueira había ingresado al portal a las 15:52, cuatro horas ANTES de que JL corriera el asistente (19:45) con la ficha abierta de antes. La edge actuó bien según el estado real (ya ingresó → no pisar la contraseña que el cliente ya usó → solo aviso), pero nadie se lo dijo a la gerencia.
- **Fix (edge v3 + mig 0383 + ficha):** (1) eliminado el envío al correo anterior — el asistente escribe SOLO a la casilla nueva; (2) template `acceso-email-actualizado` reescrito como la bienvenida completa (estructura canónica de `bienvenida-administracion`: credenciales + beneficios + link + recomendación de cambio de contraseña) con una línea del cambio de usuario; el `-aviso` queda únicamente para "ya ingresó" (contraseña intacta — es imposible reenviarla porque no se almacena en claro; solo podemos regenerarla, y pisársela a alguien que la conoce es peor); (3) el asistente re-consulta `cliente_acceso_estado` AL MOMENTO de operar, sincroniza el semáforo y anuncia exactamente qué va a recibir el cliente antes de confirmar.
- **Verificado (e2e real de ambos branches con QA efímero, 0 residuos):** nunca-ingresó → exactamente 1 mail, al nuevo, template bienvenida CON `password_temporal`; ya-ingresó → exactamente 1 mail, al nuevo, `-aviso` SIN password; **0 mails al correo viejo en ambos**; login+ficha actualizados en ambos. Live QA del asistente en el bundle deployado.
- **Pendiente derivado (Pablo, para otra etapa):** blanqueo de contraseña iniciado por gerencia para el caso "ya ingresó y la olvidó" (hoy el camino es "¿Olvidaste tu contraseña?" del login, que depende del email de acceso ya corregido).
- **Lección:** los patrones "estándar" de seguridad (avisar al mail viejo) no se importan sin pesar el caso de uso del negocio; y toda acción de gerencia cuyo efecto depende de un ESTADO debe re-consultarlo al momento de ejecutar y anunciar el efecto — el semáforo pintado N horas atrás no es el estado.

### E-GG-157 · Blanqueo de contraseña por gerencia (pedido de Pablo) + cierre del vector de escalación en las 3 edges de acceso
- **Qué es:** botón "Blanquear contraseña" en la ficha del cliente — genera una contraseña nueva (la actual deja de servir) y el cliente la recibe en su email de LOGIN vigente con formato bienvenida ("blanqueamos tu contraseña y generamos esta nueva"). Camino PARALELO y exclusivo de gerencia respecto del "¿Olvidaste tu contraseña?" del login (intacto para autoservicio). Caso de uso: el cliente ya ingresó, olvidó su clave, y llama a la gerencia.
- **Piezas:** edge `blanquear-password` (staff-gate real, calco del patrón probado de reenviar-bienvenida) + template `acceso-password-blanqueada` (mig 0384, estructura bienvenida canónica) + botón en la ficha con estado FRESCO (lección E-GG-156: anuncia si "ya ingresó" o no antes de confirmar) y disabled cruzado con Reenviar/Corregir.
- **Hallazgo §6 cerrado en el mismo chunk (sistémico, pre-existente de DGG-117):** las edges de acceso resolvían el target SOLO por `administraciones.user_id` sin validar el ROL del usuario destino → una ficha con `user_id` apuntado a un gerente permitía pisarle la contraseña (lockout) o —peor, vía corregir-email-acceso— cambiarle el email de login. Fix en LAS TRES edges (blanquear v2, reenviar-bienvenida v2, corregir-email-acceso v4): guard anti-escalación `profiles.role='administrador'` sobre el user_id resuelto + rechazo explícito de clientes en estado 'baja' (defensa en profundidad del gating que el front ya hacía).
- **Verificado (e2e real, QA efímero, 0 residuos):** password vieja 200 → blanqueo → password vieja **400** / password nueva del mail **200** (login real con ambas); 1 solo mail al login vigente con template y password correctos; ficha manipulada (user_id=gerente QA) → **409 rechazada** con la contraseña del gerente intacta; ficha legítima → 200. Live QA del botón en el bundle deployado.
- **Lección:** toda edge que muta credenciales debe validar no solo QUIÉN llama (staff-gate) sino QUIÉN es el target (rol del usuario destino) — el puntero `user_id` de una fila editable no es una autorización.

### E-GG-158 · (reporte de Pablo) Los cards de aviso del Inicio de gerencia no se podían descartar
- **Síntoma:** el banner "Emails rebotados" no tenía X y quedaba 7 días fijo (mostrando incluso rebotes históricos ya resueltos, como el del correo viejo de Nogueira del flujo eliminado en E-GG-156); los demás banners de aviso (solicitudes, pagos informados, docs del cliente, aportes de gestoría) tampoco tenían X — solo desaparecían cuando la acción de negocio vaciaba la lista.
- **Causa raíz:** los 5 widgets-banner del Inicio se diseñaron "persistentes mientras haya items", sin mecanismo de descarte. Para clientes la reiteración es deseable; el gerente debe poder decidir cuándo dejar de ver un aviso (pedido explícito de Pablo).
- **Fix:** hook compartido `useCardDismiss(key)` (localStorage por navegador — preferencia visual, no estado de negocio) + X en los 5 banners con semántica "ocultar HASTA QUE HAYA NOVEDADES": la X guarda el instante del descarte y el card solo reaparece con items posteriores — nunca se pierde un aviso nuevo. En NuevasSolicitudes, si hay pendientes pero todas descartadas NO se muestra la barra "Todo al día" (sería mentira). `AlarmasHoyWidget` queda deliberadamente sin X: su mecanismo correcto es "Postergar +N días" por alarma (ocultar recordatorios de vencimientos en bloque sería peligroso).
- **Verificado:** build limpio + live QA en prod: la X del banner de rebotados descarta el rebote real de Nogueira y NO vuelve tras recargar; un item nuevo posterior revive el card (verificado por diseño del filtro + e2e visual).
- **Lección:** todo banner de aviso permanente necesita responder dos preguntas desde el diseño: cuándo aparece Y cuándo el usuario puede decidir dejar de verlo — con la garantía de que las novedades siempre reviven el aviso.

### E-GG-159 · (reporte de JL vía Pablo) La cuenta corriente decía "servicios" en vez de QUÉ servicio se contrató
- **Síntoma:** la línea de cargo del extracto (ficha del cliente Y portal) mostraba "X 00001-00000006 · servicios" — el CONCEPTO FISCAL del comprobante (el rótulo ARCA servicios/productos) — mientras Facturación mostraba bien el detalle ("Curso de Capacitación Inicial o Formación (Pcia. de Bs. As.)"). Canon de Pablo: única fuente de verdad — todas las superficies deben dejar constancia del servicio contratado; "después de todo, todos son servicios".
- **Causa raíz:** la RPC `cuenta_corriente_extracto` (que ES la fuente única — la consumen gerencia, portal, PDF de cta cte y extracto de orden) concatenaba `c.concepto` en el branch del cargo. Irónicamente, la MISMA RPC ya tenía la cascada correcta escrita para "Saldo a favor" (W8-2): servicio → items → trámite → concepto.
- **Fix (mig 0385):** el branch del cargo usa la misma cascada canónica: `servicios.nombre` (por servicio_id) → `string_agg(items_comprobantes.descripcion)` (el detalle facturado, idéntico a Facturación) → `tramites.titulo` → concepto fiscal como último recurso. Como la descripción se deriva EN LECTURA, todos los históricos quedaron corregidos sin backfill, y las 4 superficies heredan el fix sin tocar el front.
- **Verificado:** smoke e2e (R18) con los 2 casos exactos de JL — comprobantes nº 6 y 8 muestran el nombre del curso con debe/haber/saldos idénticos; barrido integral de los 10 cargos del sistema → 0 quedaron con rótulo fiscal; smoke de overloads (R16) → 0; live QA en prod sobre la ficha real (extracto con el nombre del curso, mismos importes).
- **Lección:** cuando una RPC compone descripciones para humanos, el campo fiscal/técnico nunca es la primera opción — y si la cascada correcta ya existe en otro branch de la misma función, la inconsistencia entre branches es el smell a auditar.
- **Cierre §6 completo (mig 0386, 25/07)** — la doble auditoría a fondo (2 agentes + ejercicio e2e con clones de comprobantes reales) sobre 0385 encontró y cerró:
  - **GAP borde C:** un item con descripción vacía (`''`) hacía que `string_agg` devolviera `''` (no NULL) → la COALESCE no caía al siguiente escalón y quedaba `"X 00001-… · "` colgante. Fix: subquery con `NULLIF(trim(...),'')` + `GROUP BY` (dedupe de descripciones repetidas, orden estable por `MIN(orden)`) + filtro de NULL.
  - **Refinamientos en AMBOS laterales** (cargo + origen de saldo a favor): trim/NULLIF en los 4 escalones y en el resultado; trámites con `ORDER BY created_at, id` determinístico + coherencia de tenencia (`t.administracion_id` NULL o = la del comprobante); lateral "ori" con guard explícito `c.administracion_id = p_administracion_id`; truncado con elipsis visible (`…`) en vez de corte seco a 160.
  - **E2e de 4 casos borde** (clones rollback del comprobante real nº 6): multi-item con duplicado y blanco → `"Consultoría jurídica + Inscripción matrícula RPAC"` (dedupeado, sin blanco); sin nada → concepto; item vacío → cae limpio a concepto (GAP resuelto); item de 200 chars → 160 con `…`. Barrido 10/10 cargos reales intactos, saldos idénticos, 18 ms (R11), 0 overloads (R16), 0 residuos QA.
  - **Live QA del PORTAL del cliente** (faltaba — solo se había probado la ficha de gerencia): usuario QA efímero administrador → `/portal/cuenta-corriente` de Nogueira muestra `"X 00001-00000006 · Curso de Capacitación Inicial o Formación (Pcia. de Bs. As.)"` con saldos coherentes, consola limpia, desktop + mobile 360px. Usuario QA borrado (0 filas verificado).
  - De paso, la auditoría destapó **E-GG-160** (abajo) — una RPC rota en prod desde su origen.

### E-GG-160 · RPC `partner_rendicion_movimientos` rota desde su origen: joineaba una tabla que jamás existió
- **Síntoma:** ninguno visible — y ESO es el bug. La RPC de la rendición del portal del partner (`PartnerPortalPage` → `fetchPartnerMovimientos`) referenciaba `public.comprobante_items`, tabla que NUNCA existió (la real es `items_comprobantes`, mig 0004). Nació rota en la mig 0118 y se re-deployó rota en la 0250.
- **Por qué nunca explotó (tormenta perfecta de 3 factores):** (1) plpgsql compila cada statement recién al ejecutarlo (lección E-GG-42/R18) → las migs aplicaron sin error; (2) el guard "solo partner o staff" hace `RETURN` vacío para staff ANTES del statement roto → todo QA hecho con gerentes pasó sin tocarlo; (3) hoy no existe ningún usuario con rol partner (0 profiles con partner_id) → nadie pudo ejecutar jamás el path roto. Evidencia ejercitada pre-fix (rol-swap partner dentro de una transacción con rollback): `42P01 relation "public.comprobante_items" does not exist`.
- **Segunda bomba latente en la misma función:** las referencias sin calificar (`atribucion_id`, `fecha`…) dentro de los CTEs colisionan con las columnas de `RETURNS TABLE` (variables plpgsql) → 42702 "ambiguous" en runtime. Tampoco se observó nunca por el mismo motivo.
- **Fix (mig 0386):** reemplazo del join fantasma (que además multiplicaba filas por item y "elegía" servicio por orden alfabético vía DISTINCT ON) por la **cascada canónica de E-GG-159** (servicio → items dedupeados → trámite → concepto) en un lateral de 1 fila por atribución, y TODAS las referencias calificadas (b./d./o.). El DISTINCT ON queda como red de seguridad.
- **Verificado:** ejercicio e2e con atribución sintética sobre el comprobante real nº 6 (rollback): la sábana devuelve `serv: Curso de Capacitación Inicial o Formación (Pcia. de Bs. As.)`, label, montos y saldo running correctos; path staff sigue devolviendo vacío; 0 overloads; 0 residuos.
- **Lección:** una RPC que "nunca falló" no es una RPC probada — si su guard de rol corta antes del cuerpo, todo smoke hecho con otro rol es teatro. Toda RPC con branch por rol exige un e2e POR CADA ROL que la alcanza (acá: partner), no solo con el rol de quien la testea. Refuerza R18: el smoke debe EJECUTAR el statement, no solo aplicar la mig.

### E-GG-161 · matricula_cumple_encuesta rebotaba al SISTEMA — cron de emisión de certificados roto en silencio desde 0227
- **Síntoma:** ninguno visible (otra vez, ESO es el bug). Descubierto por el e2e de DGG-119: el checklist de Selalle quedó 3/3 y el certificado no se emitía.
- **Causa raíz:** el guard de `matricula_cumple_encuesta` (0227) exige staff O dueño, pero los llamadores internos corren sin JWT (`auth.uid()` NULL): el cron backstop `gg_campus_emitir_certificados_pendientes` (pg_cron cada 5 min) rebotaba en 42501 para TODA matrícula, y el best-effort de 0229 lo tragaba sin log útil. Compensación mutua perversa: el trigger de emisión por tilde del gerente tampoco emite (la RPC interna no tiene grant para authenticated — by design), y "lo cubría el cron"... que estaba roto.
- **Fix (mig 0391):** `auth.uid() IS NULL` = contexto interno permitido (inalcanzable desde PostgREST). Staff/dueño igual; terceros autenticados siguen en 42501. Re-disparo del backstop → primer certificado de curso real emitido (Selalle, GG-FORM-2026-859AD8).
- **Lección:** (1) un best-effort sin telemetría es un tragadero de bugs — loggear SIEMPRE el SQLERRM aunque no se propague (0229 hace `NULL` pelado); (2) toda función con guard de identidad que pueda ser llamada por cron/trigger necesita la rama de contexto sistema, y su e2e debe ejecutarse TAMBIÉN sin JWT (refuerza E-GG-160: e2e por CADA contexto que alcanza el código).

### E-GG-162 · El checklist de condiciones no se sembraba al matricular — gate de certificado salteable para matrículas nuevas
- **Síntoma:** ninguno aún (cazado por la §6 adversarial de DGG-119 ANTES de que existiera una víctima real). El auditor probó e2e: alta nueva → 0 filas de checklist → responder SOLO la encuesta → certificado emitido (sin examen, sin pago).
- **Causa raíz:** el seed de matricula_condiciones de la mig 0390 fue one-shot histórico; ningún trigger sembraba las filas al matricular (las syncs canónicas insertan la fila recién AL CUMPLIRSE) y el gate de emisión cuenta sobre las filas existentes → gate más laxo que lo configurado para todo alta futuro. El aviso de retención (AFTER UPDATE) tampoco disparaba jamás para nuevas.
- **Fix (mig 0392):** trigger AFTER INSERT en curso_matriculas (siembra todas las condiciones activas + put-al-día con syncs canónicas), trigger en curso_condiciones_config (activar una condición siembra en todas las matrículas del curso), upsert también en la rama no-completo del sync de pago. Además §6 B#3: `matricula_set_estado_pago` al salir de pago_completo destilda la condición aunque el tilde fuera manual (la decisión explícita del gerente manda, auditada), y `curso_registrar_pago` compara monto vs precio_lista (un pago parcial ya no declara pago completo).
- **Verificado:** re-ejercicio del escenario exacto del bypass con rollback: filas sembradas 3/3, solo-encuesta NO emite, aviso dispara (2 mails + marca), certificado retenido.
- **Lección:** un seed one-shot en una mig NUNCA reemplaza el invariante estructural (trigger). Si el gate cuenta "filas cumplidas vs filas existentes", la existencia de las filas ES parte del contrato de seguridad.

### E-GG-163 · Reconciliación de asistencia Zoom: el "matching por nombre" nunca existió y sellaba en vacío
- **Síntoma (reporte de Pablo, 2ª vez):** encuentro del 27/07 finalizado, 34 personas en la sala, 82 eventos webhook registrados — y 0 presentes tras el cierre. La reconciliación figuraba "hecha" (asistencia_reconciliada_at seteado a los 26 segundos del fin).
- **Causa raíz (doble):** (1) el diseño de E-GG-145 prometía matching por NOMBRE contra el reporte de Zoom, pero la RPC solo matcheaba customer_key + email — y los invitados de Zoom llegan SIN email (lección ya documentada en E-GG-145) → 0 matches garantizado para todo el que entra por link crudo. (2) La RPC sellaba `asistencia_reconciliada_at` SIEMPRE, aun con payload vacío o 0 matches → `zoom_encuentros_pendientes_reconciliar` (filtra por NULL) nunca reintentaba.
- **Fix definitivo (mig 0393):** (a) 3er escalón de matching por NOMBRE normalizado (sin tildes/signos, tokens en cualquier orden, contención de conjuntos, y SOLO con candidato único — ambiguos como "Olga" van a pase manual, jamás se adivina); (b) la RPC fusiona SIEMPRE los eventos webhook propios (`curso_encuentro_zoom_eventos`: user_name + join/leave que ya viven en nuestra BD) con el reporte de la API → independencia total del reporte de Zoom; (c) sellado honesto: no se sella sin participantes ni antes de 10 min del cierre — el cron de 15 min reintenta hasta reconciliar de verdad.
- **Verificado:** ensayo en rollback con los datos reales del 27/07 (payload de API vacío = peor caso): 6/6 alumnos matcheados por nombre desde los eventos propios — Althabe ("renan agustin althabe. mat 333") 92min ✓, Palópolo (tilde) ✓, Marsiglia (orden invertido) ✓, Webb ✓, Marzal ✓ presentes; Amado 11min → presente=false (umbral 50% funcionando). Corrida EN FIRME: 6 asistencias registradas, 34 externos FundPlata a pase manual, sellado. Retroactiva del 22/07: sin eventos webhook de esa fecha (el log nació después) → re-sellado con su presente manual original.
- **Lección:** si un diseño promete un mecanismo (matching por nombre), el e2e de cierre debe ejercitar ESE mecanismo con el dato real adverso (invitados sin email), no el camino feliz. Y una marca de "hecho" que se estampa sin verificar resultado convierte cualquier fallo en permanente e invisible.

### E-GG-164 · (reporte de JL) Wizard de activación: "Siguiente / Comenzar proceso no se habilita" — el candado del chequeo anti-duplicado quedaba clavado
- **Síntoma:** en el paso 2 (Documentación), con un doc marcado Incorrecto + opción elegida + observación completa, el botón Siguiente (Pedir documentación y avanzar) o Comenzar proceso (Rechazar) seguía deshabilitado. Flujo bloqueado.
- **Causa raíz (NO era la validación):** la validación del paso 2 se probó correcta montando el componente real (reproducción imposible por esa vía). El bug real: `comenzarProceso()` (gate anti-duplicado DGG-89) hacía `await buscarTramiteDuplicado()` sin timeout ni catch; si la request quedaba colgada (red stalled / auth-lock de supabase-js, familia E-GG-155 que ya sufrió JL), el `finally` nunca corría y `verificandoDup` quedaba `true` PARA SIEMPRE — y ese flag deshabilita el ÚNICO botón de avance de TODOS los pasos. Un solo click colgado en el paso final contaminaba el wizard entero, incluido el paso 2. Reproducido 1:1 stalleando la request.
- **Fix:** `Promise.race` con timeout de 8s + catch → fail-open del soft-gate (su peor degradación = no mostrar el aviso de duplicado, que es lo que ya pasaba con `dup.ok=false`); `verificandoDup` siempre vuelve a false.
- **Hallazgo crítico de la §6 SOBRE el propio fix (cerrado en el mismo commit):** con el timeout garantizado, si el usuario clickeaba Atrás durante los 8s, el `goNext()` diferido (closure con `esPasoFinal=true` congelado) lanzaba el ProcesadorFinal desde otro paso con el estado a medio editar. Fix: Atrás/Cancelar/dots del Stepper deshabilitados mientras corre el chequeo.
- **Verificado:** e2e BD del circuito completo con rollback forzado (solicitud sintética → `solicitud_activar` crea cliente+trámite ✓ → `tramite_pedido_doc_crear` crea pedido+item ✓ + 4 emails encolados ✓; banner/push requieren el usuario del portal, que en el flujo real el ProcesadorFinal crea ANTES del pedido — orden verificado); live QA en prod con la solicitud real de Catelli: ambos casos habilitan (Siguiente ✓, Comenzar proceso ✓), cancelado sin procesar, solicitud intacta.
- **Lección:** todo `await` de red detrás de un flag de loading que deshabilita UI necesita timeout + catch (el patrón puede existir en otros modales — sweep pendiente si Pablo lo pide); y un fix que convierte un colgado en un disparo diferido debe congelar la navegación mientras espera.

### E-GG-165 · (reporte de JL) Resumen de cuenta del partner ordenaba con lo más nuevo abajo
- **Síntoma:** la sábana del partner (gerencia + portal FundPlata) listaba cronológico ascendente → lo más nuevo quedaba al fondo. Las cajas ya habían sido corregidas (0341) y el extracto de cta.cte también (E-GG-122); este componente quedó fuera de aquel barrido.
- **Causa raíz:** la RPC `partner_sabana` devuelve ASC porque el saldo corrido por fila se computa con window function en ese orden (correcto y necesario), y `SabanaPartner.tsx` pintaba el array tal cual llegaba.
- **Fix (patrón E-GG-122, presentación-only):** `displayLineas = useMemo(() => [...lineas].reverse(), [lineas])` en tabla + exports PDF/XLS. La RPC y los KPIs NO se tocan (`kpis.saldo` sigue leyendo `lineas[length-1]` sobre el array ASC). Un solo componente compartido → gerencia y portal heredan a la vez.
- **Verificado:** §6 adversarial OK sin hallazgos (censo de consumidores completo, exports coherentes, bordes [] y [x]); live QA: primera fila 28/07 con saldo $711.167,75 == KPI, 13 filas, mobile 360 sin overflow.
- **Lección:** los barridos de presentación (newest-first) deben inventariar TODAS las superficies con saldo corrido, no solo las reportadas — la sábana del partner esperó 3 semanas a que JL la viera.

### E-GG-166 · (reporte de JL) Certificado del curso con el nombre del alumno cortado a la mitad
- **Síntoma:** el PDF del certificado de Selalle (GG-FORM-2026-859AD8) mostraba solo la mitad superior de las letras del nombre.
- **Causa raíz:** en `CertificadoPremium` el div del nombre es el ÚNICO hijo flex con `overflow:hidden` (guard horizontal deliberado) → su "automatic minimum size" es 0 → cuando el título del curso envuelve a 2+ líneas (el asincrónico: 56 chars), flexbox le descarga TODO el espacio negativo de la columna de altura fija y recorta los glifos. En el raster del PDF es peor: `skipFonts` (E-GG-95) usa fuentes fallback más anchas que pueden sumar una línea extra al título. Por eso "en pantalla se veía bien".
- **Fix:** (1) `flexShrink: 0` en el div del nombre — el excedente se derrama al padding inferior (300px de aire), verificado sin pisar firmas; (2) fontSize por ancho ESTIMADO determinístico (mayúscula ~0.75em, minúscula ~0.52em, espacio ~0.35em, margen 10%, piso 30px — calibrado con harness de 11 escenarios) que reemplaza el truncado silencioso con "…" de nombres largos; la fórmula por CANTIDAD de caracteres se descartó porque los nombres en MAYÚSCULAS truncaban igual (~40% más anchos).
- **Regenerar/re-descargar:** el PDF del curso se genera on-the-fly en cada descarga (pdf_storage_path NULL, el email manda link, no adjunto) → con el fix deployado toda descarga sale bien. Las superficies YA existían: el alumno conserva SIEMPRE el botón en Campus → nodo Certificado, y gerencia tiene Vista previa + Descargar por alumno en la tab Alumnos del curso. Sólo hay un certificado emitido en todo el sistema (el de Selalle) — no hay PDFs almacenados que remediar.
- **Verificado:** harness empírico (baseline reproduce el corte de 13.3px; con fix 0px clipeado en los 11 escenarios, 0 truncados hasta 41 chars mayúsculas, firma intacta hasta título de 3 líneas); live QA en prod: vista previa de Selalle con el nombre completo.
- **Lección:** en una columna flex de altura fija, `overflow:hidden` convierte a ese hijo en la válvula de escape del layout — todo hijo que no deba achicarse necesita `flexShrink: 0` explícito; y cualquier auto-ajuste de fuente para un raster con fallback de fuentes debe ser determinístico por contenido, nunca por medición DOM.

## E-GG-167 · Los campos ocultos por condición SÍ viajaban en el submit (motor de formularios) (2026-07-31)
- **Síntoma:** ninguno visible — bug LATENTE del motor detectado al diseñar DGG-123. El docstring de `condition` en `src/services/api/formularios.ts` prometía "cuando el campo no es visible, queda excluido del payload", pero ni el runner ni el edge lo implementaban: `onSubmit` mandaba TODO `data` + TODOS los files, y el edge persistía `datos` crudo. Con el switch PF/PJ hubiera significado envíos de personas físicas arrastrando datos societarios remanentes (y al revés), y valores stale de campos ocultos (cónyuge con estado_civil oculto).
- **Causa raíz:** contrato documentado pero jamás implementado (documentación aspiracional). Sub-hallazgo de la doble auditoría: la visibilidad tampoco cascadeaba — un campo condicionado a otro campo OCULTO seguía visible si el valor remanente matcheaba (caso real: PF elige "casado", carga cónyuge, cambia a PJ → estado_civil se oculta pero cónyuge seguía en pantalla y viajaba).
- **Fix:** runner filtra `data`+`files` por `isFieldVisible` al enviar + visibilidad recursiva con cascada (dep oculto ⇒ vacío) + secciones sin campos visibles no se renderizan. Edge submit-formulario v12: mismo strip con cascada ANTES de `validarIdentidadObligatoria` (auditor A: si corría después, un payload crafteado podía "pasar" identidad con valores en campos ocultos que luego se borraban) y sin memo en la recursión (el memo era order-dependent bajo ciclos de condición; evaluación fresh = paridad exacta con el runner).
- **Hallazgos hermanos de la doble/triple auditoría (cerrados en 0398):** B#1 contradicción veto-CUIT (0397) vs bloqueo duro de `solicitud_activar` (E-GG-130) — email compartido titular/sociedad con CUIT distinto moría con el 23505 genérico; ahora el mensaje explica que son clientes separados (DGG-123) y pide un email propio (el email es el login). B#2 responsable_nombre/apellido viajaban en el input pero el INSERT los ignoraba. B#3 `domicilio_empresa` no entraba a `direccion` en los triggers de backfill (patrón E-GG-114). B#4 cliente logueado declarando PJ ya no backfillea el CUIT de la empresa sobre su cuenta. C#1 `provincia` caía a "Otros datos" en la máscara PJ del gestor.
- **Lecciones:** (1) todo contrato en un docstring se verifica contra el código antes de apoyar un feature encima — "documentado" ≠ "implementado"; (2) al agregar un veto/regla de identidad en UNA función (match), buscar TODAS las funciones hermanas que dupliquen esa lógica (guard de activar) — la doble auditoría lo cazó en 30 min; (3) visibilidad condicional sin cascada es un bug esperando un switch de dos ramas.

## E-GG-168 · Re-suscripción push con conflicto fallaba en silencio (sin policy UPDATE) (2026-07-31)
- **Síntoma:** ninguno reportado — bug LATENTE cazado por el auditor adversarial del ACL sweep (DGG-125). `push.ts` hace `.upsert(..., { onConflict: 'user_id,endpoint' })` sobre push_subscriptions; la tabla tenía policies INSERT/SELECT/DELETE pero NO UPDATE → cuando el upsert caía en conflicto real (mismo usuario re-activando notificaciones en el mismo browser con keys rotadas), RLS bloqueaba el UPDATE con 42501 y la suscripción quedaba con keys viejas (pushes rotos para ese dispositivo).
- **Doble descubrimiento del refutador:** (1) Postgres exige el privilegio UPDATE en PARSE-TIME de un INSERT..ON CONFLICT aunque no haya conflicto — probado empíricamente en el Postgres vivo; por eso el plan original de revocar el UPDATE "muerto" hubiera roto "Activar notificaciones" para todos (la única línea refutada de 126). (2) El grant estaba vivo pero la POLICY faltaba: el conflicto real ya fallaba hoy.
- **Fix (mig 0401):** conservar el grant UPDATE + policy `push_sub_owner_update` (USING/WITH CHECK user_id = auth.uid()). e2e con rollback: upsert con conflicto real actualiza las keys ✓.
- **Lecciones:** (1) `.upsert()` exige privilegio UPDATE aunque el conflicto nunca ocurra — al auditar grants, buscar TODOS los upserts del front antes de tocar UPDATE; (2) "grant sin policy" tiene dos remedios según el caso: revocar el grant (permiso muerto) o AGREGAR la policy (feature rota en silencio) — el upsert es la pista de cuál corresponde; (3) la refutación adversarial pre-aplicación pagó exactamente donde el análisis mecánico era ciego.

## E-GG-169 · Avisos de demora de Gmail clasificados como rebote → alertas falsas a gerencia (2026-08-02)
- **Síntoma:** Pablo recibió "Rebotó un email a qa-pfpj-dgg123@qa.gestionglobal.ar" (01/08 18:31 ART, y su gemelo qa-pf a las 20:01) sin ningún testeo en curso. Eran ecos diferidos del live QA de DGG-123 (31/07): los mails "Recibimos tu formulario" a direcciones QA de un subdominio sin MX no rebotan en el acto — Gmail los reintenta ~47 hs y manda avisos de DEMORA ("Hubo un problema temporal… seguirá intentando 44 horas más") que el harvester marcó `bounced` y notificó a ambos gerentes.
- **Causa raíz:** el `email-bounce-harvester` clasificaba por dato máquina (`Action:`/`Status:` del part `message/delivery-status`); esos DSN en español vinieron SIN part parseable → `action`/`statusCode` null → default `bounced` (que dispara `notify_all_gerentes`), cuando el diseño DGG-117 dice que las demoras van a `delivery_delayed` (sin alerta). Con un cliente real, un servidor caído 2 hs habría sonado como "corregí su casilla" siendo que el mail llegaba después.
- **Fix (edge v5, sin migración):** heurística `DELAY_PHRASES` (ES/EN) sobre el texto humano SOLO cuando no hay dato máquina. Blindajes de la §6 adversarial (agente B): `FAIL_PHRASES` con precedencia (evidencia declarativa de falla gana; ojo que los avisos de demora contienen futuros condicionales tipo "si la entrega falla definitivamente" — jamás matchear "permanent"/"definitiv" a secas), narrativa cortada en el delimitador de cita (el DSN adjunta NUESTRO mensaje original y su copy no debe clasificar su propio rebote), subject sin el eco "Undeliverable: <asunto>", guard anti-degradación (una demora no pisa `bounced`/`complained`), y gate `esDsn` con variantes exim/qmail ("returning message to sender", "failure notice"). B#6 (`!dsPart`) evaluado y NO aplicado: mataría la heurística si Gmail entrega el delivery-status como attachmentId, que es plausiblemente el caso real. Test harness local 18/18 (réplica exacta del bloque de decisión con los DSN reales del 01/08 + adversariales).
- **Limpieza:** borradas las 2 filas `sent_emails` QA para que los DSN de falla definitiva (~48 hs post-envío) no matcheen ni re-alerten. Verificado 0 filas `%@qa.gestionglobal.ar` en sent_emails/email_queue/formulario_submissions.
- **Lecciones:** (1) emails QA siempre a dominios que rebotan DURO (`@gestionglobal.test` falla en minutos con 1 DSN) o a un alias propio — un subdominio inexistente de dominio real (`@qa.gestionglobal.ar`) genera 2-3 DSN escalonados durante 48 hs post-limpieza; (2) un mail ya enviado no se "borra" con la limpieza QA: la fila `sent_emails` queda matcheable 7 días por el harvester — si el destino QA va a rebotar, borrar también esas filas al limpiar; (3) en clasificadores con default agresivo (bounced→alerta), la rama "sin datos" merece heurística propia: el caso degenerado no era raro, era el caso real; (4) GAP menor pre-existente anotado por el agente C: `delivery_delayed` es terminal hoy (Gmail no emite DSN de éxito y nada asciende a `delivered`) → badge "Demorado" eterno si el mail al final llega; mejora futura (expiración en UI o sweep).

## E-GG-170 · Solicitud "válida" sin su comprobante obligatorio — el fallo de Storage se tragaba en silencio (caso Rodríguez) (2026-08-02)
- **Síntoma:** hubo que rechazar la inscripción de Rodríguez Mónica Victoria (curso-actualizacion, 31/07 09:44, desde PC Windows) porque llegó sin el comprobante de pago, pese a que el campo `file` es `required: true`. Única de 16 inscripciones históricas sin fila en `formulario_adjuntos` y sin objeto en Storage. Ella reintentó el 01/08 desde el celular y esa segunda submission SÍ trajo el comprobante (jpg 7 MB, quedó `pendiente`).
- **Causa raíz (cadena de 3 huecos):** (1) el edge `submit-formulario` insertaba la submission ANTES de subir los archivos, y un fallo del upload hacía `console.error + continue` devolviendo `ok:true` — la validación de required pasó porque el archivo SÍ venía en el payload; (2) Storage rechazó su archivo (bucket `form-adjuntos`: 10 MB + whitelist jpeg/png/webp/pdf/xls — el intento violó tamaño o formato; los logs de edge sólo retienen 24 hs); (3) el front no validaba tamaño/tipo y el campo no definía `accept` → cualquier archivo viajaba. Resultado: solicitud "válida" sin adjunto, usuario viendo "Formulario enviado ✓".
- **Fix (edge v13 + runner + mig 0402, §6 doble con 3 agentes + adversarial):**
  - **Orden invertido**: validar (tamaño/MIME espejo del bucket, con fallback por extensión B#7a y rechazo de 0 bytes B#1d) → subir a Storage con `submissionId` pre-generado → recién ahí INSERT (los triggers del insert tienen side effects irreversibles: solicitud + acuse por mail + avisos). Falla de upload ⇒ remove() de lo subido + 502 claro; la submission ni nace. Colisión de nombre desambiguada con índice (A#11); cap `max_files ?? 1` server-side (B#3c); sólo se aceptan files de campos `file` del schema.
  - **Backstop**: adjunto subido cuya fila no se puede registrar (ventana de crash B#1b parcial) → `notify_all_gerentes('formulario_adjunto_huerfano')` con path y submission.
  - **Front**: `accept` por defecto (en iOS además fuerza la transcodificación HEIC→JPEG), rechazo en el picker con toast accionable + cinturón en `validate()`.
  - **mig 0402**: (a) B#1c — race del voucher: `crear_tramite_desde_submission_auto()` seguía EN SILENCIO sin descuento si el voucher se agotaba entre el pre-check del edge y el INSERT (solicitud a precio completo y, con voucher 100%, sin comprobante) → ahora RAISE EXCEPTION; smoke e2e con rollback: voucher agotado aborta sin dejar submission, flujo normal intacto (solicitud + acuse + $80.000). (b) B#2c — policy `form_adj_insert` dejaba INSERT anon en el bucket → eliminada (único escritor legítimo: el edge con service_role). (c) bucket `pedidos-doc-cliente` sin límites → 20 MB + whitelist.
- **Barrido de hermanos (agente C, 21 call sites de upload):** aplicados en el mismo chunk — helper compartido `src/lib/adjuntos.ts` + espejos cliente en InformarPagoModal (el gemelo del caso: comprobantes de pago de clientes, `accept` que dejaba pasar formatos que el bucket rechazaba), PedidosDocPanel, AccesoExternoPage (además: adjuntos con error ya NO se excluyen en silencio al enviar el avance + fix del índice stale-closure que marcaba el ítem equivocado), TramiteDetailPage, NuevoMovimientoModal/MovimientoAdjuntosButton (motivo del fallo visible), plantillas de certificado (5 MB), PartnerPortal (accept .pdf + tope). `tramix-doc-proxy`: upload sin chequear + cache upserteado igual → ahora `STORAGE_ERROR` explícito sin envenenar el cache. Transversal: `humanizeError` traduce los 4 errores crudos de Storage.
- **Hallazgos aceptados/documentados sin fix:** B#6 (`!dsPart`→ n/a acá), B#4 sniff de magic bytes (opcional), off-by-2 bytes en archivo de exactamente 10 MB (cosmético), huérfanos de Storage si `remove()` falla (sólo costo, se loguea), reconciliador para la ventana de crash post-insert (backlog).
- **Lecciones:** (1) en flujos con triggers de side effects irreversibles, TODO lo falible (uploads, llamadas externas) va ANTES del insert que los dispara — "insertar y después completar" fabrica registros a medias invisibles; (2) un `catch { console.error; continue }` en un paso OBLIGATORIO del flujo es un bug de diseño, no manejo de errores: o aborta con mensaje claro o escala a gerencia; (3) todo límite de bucket necesita su espejo cliente Y su pre-validación server (el rechazo de Storage es el ÚLTIMO recurso y es mudo); (4) la validación autoritaria en el trigger debe FALLAR ruidoso cuando el dato no cierra (voucher NOT FOUND silencioso = variante racy del mismo bug); (5) el barrido de hermanos paga: el gemelo exacto (comprobantes de pago del portal) estaba a un archivo >10 MB de repetir el incidente con plata real.

## E-GG-171 · Secuelas §6 ultra del merge E-GG-170: accept `image/*` bloqueaba iPhones, el MIME normalizado no viajaba al bucket (regresión activa de la 0402) y faltaban espejos (2026-08-02)
- **Contexto:** auditoría multi-agente (3 dimensiones + refutación adversarial por hallazgo, 11 agentes) sobre el diff completo del merge E-GG-169/170 ANTES de deployar el front. 8 hallazgos confirmados, 0 refutados; los críticos se fixearon en el mismo chunk.
- **Hallazgo 1 (crítico) · accept custom `image/*`:** `FileUploader` pasaba `field.accept` crudo al `<input accept>`. El form vivo `consultoria-juridica` definía `["application/pdf","image/*"]` en sus 2 campos file (uno required): `image/*` admite HEIC → Safari iOS NO transcodifica la foto de cámara (solo lo hace cuando el accept no la admite — el racional documentado del propio default E-GG-170) → el runner y el edge la rechazan siempre → campo required imposible desde un iPhone con cámara en formato default (ídem HEIF Samsung y GIF/BMP/TIFF desktop). **Fix:** `sanearAccept()` en el runner (interseca con la whitelist, expande `image/*` a `.jpg,.jpeg,.png,.webp`, descarta lo no soportado, fallback al default) + mig 0403 (cinturón de datos: UPDATE del schema con snapshot; 0 `image/*` restante en todos los forms).
- **Hallazgo 2 (crítico) · el MIME normalizado no viajaba — regresión ACTIVA de la mig 0402:** `mimeDeArchivo()` acepta por extensión archivos con `File.type` vacío/octet-stream (caso B#7a real: providers Android/Windows con .xlsx), pero el upload subía el `File` CRUDO — y **storage-js con cuerpo Blob/File arma multipart e IGNORA la opción `contentType`** (verificado en el SDK 2.106.0 instalado): el content-type que valida la whitelist del bucket sale de `File.type`. Con la whitelist nueva de `pedidos-doc-cliente` (mig 0402, ya aplicada) un .xlsx que el picker aceptaba moría en 415 con mensaje contradictorio y sin workaround — un flujo cliente que la semana anterior funcionaba. `uploadComprobantePago` era peor: forzaba `application/octet-stream` explícito contra la whitelist de `pagos-reportados`. **Fix:** helper `normalizarAdjunto(f)` en src/lib/adjuntos.ts (re-envuelve `new File([f], f.name, {type: mimeDeArchivo(f)})` — pasar contentType NO alcanza) aplicado en los 7 uploads a buckets con whitelist: tramitePedidosDoc, tramites (adjunto + doc final), finanzas, pagosReportados, certificado-esquemas, formularios-admin (previews).
- **Hallazgo 3 (menor) · validation.min/max sin espejo en el runner:** el edge 422aba `ano_egreso` fuera de 1950-2050 (matriculacion-rpac) recién después de codificar y subir ~9 adjuntos. Fix: min/max en `validate()` + atributos HTML en el Input number.
- **Hallazgo 4 (menor) · off-by-two en el pre-check de tamaño del edge:** `floor(len*3/4)` sin descontar padding rechazaba un archivo de EXACTAMENTE 10.485.760 bytes con «pesa 10.0 MB y el máximo es 10 MB». Fix: `bytesDeBase64()` exacto (edge v14).
- **Hallazgo 5 (menor) · sin tope total del payload:** 9-11 archivos rozando el cap → body ~120 MB → 546 WORKER_RESOURCE_LIMIT tras minutos, error opaco (probado empíricamente contra el edge real: 80 MB OK, 120 MB muere). Fix: tope combinado 60 MB raw en runner + edge con mensaje accionable.
- **Lecciones:** (1) un accept custom más laxo que la whitelist del bucket REINTRODUCE el bug que el default vino a tapar — sanear en el componente, no confiar en los datos; (2) `contentType` en `.upload()` con File/Blob es un no-op de storage-js: toda whitelist de bucket exige re-tipar el CUERPO con el MIME efectivo del espejo (patrón `normalizarAdjunto`); (3) agregar whitelist a un bucket productivo (hardening 0402) es un cambio de CONTRATO para todos sus uploaders — barrer los call sites en la misma mig; (4) los límites per-ítem necesitan su tope agregado (la clase de fallo "muchos válidos = inválido silencioso").

## E-GG-172 · Harvester: el DSN de falla definitiva por expiración de reintentos quedaba como "demora" eterna — gerencia nunca se enteraba del mail muerto (2026-08-02)
- **Síntoma (latente, cazado por la §6 ultra):** cuando Gmail agota los ~47 hs de reintentos emite el DSN final con `Action: failed` + `Status: 4.4.7` — la secuela directa esperada del propio E-GG-169. La cláusula v5 «status 4xx → delivery_delayed» no verificaba el action: ese DSN quedaba como demora, NO disparaba `notify_all_gerentes`, y al marcar `dsn_msg_id` el mensaje no se reprocesaba jamás. Como es el ÚLTIMO DSN de la cadena, ningún otro marcaba bounced: mail muerto + badge "Demorado" eterno + gerencia ciega. El falso negativo simétrico (y peor) del falso positivo que E-GG-169 arregló. El comentario del código («el rebote definitivo posterior llega como DSN nuevo y sí marca bounced») era falso para esta clase — misma clase que casilla llena prolongada (4.2.2).
- **Fix (harvester v6):** guarda `bounce.action !== 'failed'` en toda la cláusula de demora — `Action: failed` es TERMINAL y clasifica bounced sin importar que el status sea 4xx. FAIL_PHRASES/DELAY_PHRASES intactas (solo corren sin dato máquina). Deploy v6 ACTIVE.
- **Lección:** en clasificadores de DSN el `Action` (intención declarada del MTA) tiene precedencia sobre la clase numérica del `Status`: un 4xx con action=failed no es transitorio, es la lápida.

## E-GG-173 · Campo "Adjuntar archivo" fantasma en curso-formacion — arrastre accidental del builder guardado sin renombrar (2026-08-06)
- **Síntoma (reporte Pablo):** en `/formulario/curso-formacion`, tras Provincia (sección "Tus datos"), aparecía un campo file "Adjuntar archivo" opcional que no correspondía — el único upload legítimo es el comprobante de pago de la sección "Pago".
- **Forense:** el campo tenía la key `file_10` — el name AUTOGENERADO que el builder asigna a un campo recién arrastrado de la paleta. Entró en una sesión de edición del builder del 27/07 (guardado posterior al snapshot v4) y pasó inadvertido tres razones: opcional (no bloqueaba a nadie), al final de una sección larga, y el validador del builder no objetaba nada (name válido y único). 0 usos en producción: ningún adjunto con `field_name='file_10'`, ninguna submission con la key. El barrido por regex de names autogenerados confirmó que era el ÚNICO caso en toda la plataforma.
- **Fix:** mig **0408** (snapshot + remoción del campo; verificado: "Tus datos" termina en Provincia, el único file del form es `comprobante_pago_inscripcion`, 0 `file_10` en todos los forms). **Cinturón de causa raíz** en `validarSchema` del builder: warning `name_autogenerado` ("¿es un campo agregado por accidente? Renombralo o eliminalo") para keys tipo `file_N`/`text_N`/etc., + de paso el warning `default_huerfano` que quedó sugerido en DGG-126 (valor inicial que ya no coincide con ninguna opción).
- **Lección:** los names autogenerados del builder son un smell de "campo que nadie pidió" — el validador debe marcarlos ANTES de publicar; un campo opcional fantasma no rompe nada, y justamente por eso puede vivir semanas frente a los solicitantes.

**Anexo E-GG-173 (2026-08-06) · Auditoría §6 multi-hipótesis posterior (11 agentes, reproche de Pablo mediante):** el cierre original recortó el canon (una sola hipótesis de barrido, sin agentes). La auditoría completa —drift schema-vivo-vs-snapshots, anomalías de contenido en todos los forms, uso real vs schema— confirmó 5 hallazgos (todos menores tras refutación), aplicados en la mig **0409**: (1) **schema_draft fantasma en curso-formacion dejado por MI propia sesión QA de E-GG-173** (el autosave del builder persistió el checkbox de prueba; un "Guardar versión" lo publicaba — la "limpieza a 0" no cubría los drafts → agregado al checklist QA); (2) costos de curso-formacion sin importe con comprobante required (génesis REFUTADA: no es del 27/07 — configuración deliberada de junio, el arancel de Formación no existe en la plataforma; **RESUELTO por decisión de Pablo 2026-08-06**: queda COMO ESTÁ — el comprobante sigue required y el importe NO se publica a propósito (los aranceles de Formación varían según el cliente y se comunican por canal directo); no es un gap sino una decisión de negocio del caso específico); (3) certificado-rpac mostraba el value interno crudo "5_dias_habiles" → "5 días hábiles"; (4) estado_civil de matriculación con options snake_case crudas → etiquetas humanas con las conditions del cónyuge actualizadas en el mismo paso; (5) total_envios inflado por limpiezas QA (el contador solo tenía trigger de INSERT) → recompute global + trigger de decremento simétrico (SECDEF, R17). E2e de humo post-0408 con rollback ✔ y 3 hipótesis descartadas con racional forense (titular de cuenta, submission pendiente del 31/07 = el caso Rodríguez ya conocido, verificaciones en cero). Lección del anexo: la limpieza QA debe barrer también `schema_draft` (el builder auto-guarda borradores con la sesión activa), y el canon §6 no se recorta por tamaño del fix.

## E-GG-174 · Balance mensual descuadrado para siempre tras revertir un movimiento (pre-existente, cazado por la §6 de DGG-129) (2026-08-06)
...
## E-GG-176 · La reconciliación de asistencia Zoom excluía TODAS las sesiones compartidas + descartes silenciosos sin customer_key (2026-08-07)

**Contexto:** al cargar DGG-131 (10 encuentros compartidos RPAC↔CABA — el 100% del calendario futuro de ambos cursos), la auditoría §6 con refutación confirmó que la red post-reunión de E-GG-145 **nunca cubrió el modelo F11 de sesiones compartidas**. Pre-existente desde F11 (jul 2026); pasó a crítico porque ahora TODAS las fechas son compartidas.

**Causa raíz (triple exclusión + agravante):** (a) el disparo de `zoom-reconciliar-asistencia` en `meeting.ended` vivía SOLO en la rama standalone del webhook; (b) `zoom_encuentros_pendientes_reconciliar` (cron 15 min) filtraba `sesion_compartida_id IS NULL`; (c) el edge resolvía el meeting SOLO contra `curso_encuentros.zoom_meeting_id` — los meetings compartidos viven en `encuentro_sesiones_compartidas` y los espejos tienen NULL, así que ni la invocación manual servía. **Agravante:** la rama compartida de `participant_joined/left` era `if (uuid) {rpc}` sin else — sin fallback por email ni log: un alumno entrando por link crudo/app nativa era invisible en vivo Y ningún proceso posterior lo rescataba (el mismo patrón que E-GG-145 erradicó del standalone; precedente real: caso Althabe 22/07).

**Fix (mig 0412 + zoom-webhook v9 + zoom-reconciliar-asistencia v3):** RPC `encuentro_sesion_reconciliar_asistencia` (reconcilia el reporte oficial en TODOS los cursos enganchados reutilizando el matching standalone por espejo) + `zoom_sesiones_pendientes_reconciliar` (barrido del cron) + `encuentro_sesion_zoom_evento_por_email` (fallback + log en espejos) + el webhook dispara reconciliación también en la rama compartida + el edge resuelve meetings de sesión. **De paso** (mismos hallazgos §6): `encuentro_sesion_zoom_evento` ganó las protecciones monotónicas de mig 0373 que solo tenía el standalone (fuente no degrada mixto/zoom_report, tiempo GREATEST, presente nunca baja); `marcarAsistencia` fija `fuente` (un tilde manual sobre fila zoom_auto ya no puede ser revertido por el próximo webhook); `recompute_asistencia` con guard de no-op (fin de los 38 updated_at fantasma por tanda de encuentros) y respeta el tilde manual de staff (`cumplida_por NOT NULL` no se baja); `gg_encuentros_recordatorio_diario` dedupea por persona+sesión (matriculado en ambos cursos = 1 solo mail 8AM); checklist de condiciones con desempate estable por `created_at` (los órdenes reales traen empates 0,0,1,1,2 y el orden visual cambiaba al tildar).

**Verificación:** e2e con rollback forzado (5 tests): barrido detecta sesión terminada ✔ · join anónimo loggea en ambos espejos ✔ · reconciliación por sesión marca `presente/zoom_report` y sella en AMBOS cursos ✔ · recordatorio con matrícula doble = 1 mail ✔ · no-op no toca updated_at + tilde manual sobrevive al recompute ✔. R16: 0 overloads.

**Lecciones:** (a) toda red de seguridad nueva (E-GG-145) debe barrer TODAS las variantes del modelo (standalone Y compartido) — el gap nació porque la reconciliación se construyó después de F11 sin mirar la rama de sesiones; (b) los fixes de robustez (0373 monotónico) también deben aplicarse a las variantes espejo de la RPC, no solo a la base.

## E-GG-177 · Subida de gestoría perdida en silencio: upload inmediato + envío en dos pasos sin persistencia (caso Spotti) (2026-08-11)

**Síntoma:** la gestoría insistía en que subió el acuse del trámite TRM-2026-00030 (Lucas Spotti) y "no es la única vez que nos reclaman"; en la plataforma no había registro ni llegó el aviso de moderación. **Evidencia forense:** dos copias byte-idénticas del mismo PDF en `gestor-uploads` bajo el token de Spotti — la del 06/08 17:24 UTC **huérfana** (sin línea de tracking) y la del 10/08 14:27 (3 min después del reaviso de JL) completa. La gestoría decía la verdad: subió el 06/08 y re-subió cuando le reclamaron.

**Causa raíz:** en el panel `/externo/:token`, `onFilesPicked` sube el archivo al bucket **apenas se elige** (tilde verde al completar), pero el avance recién existe cuando `enviar()` llama a `gestor_cargar_avance` con descripción ≥3. El borrador vivía solo en `useState`: una recarga entre medio lo mataba en silencio, dejaba el archivo huérfano y el gestor creía haber enviado (la señal visual decía "listo"). El aviso a gerencia nace en el trigger de la línea → sin línea, cero avisos. Log de aperturas: el 06/08 la página se **recargó 26 segundos** después del upload.

**Fix (3 capas, migs 0414+0415 + AccesoExternoPage):**
1. **UX:** banner ámbar `role=status` "cargados pero TODAVÍA NO SE ENVIARON" (copy por estado: subiendo / falta descripción / listo) + guard `beforeunload` con borrador pendiente (sin `!enviando`: cubre la ventana in-flight).
2. **Borrador persistente** en localStorage por token (iOS Safari no dispara beforeunload — WebKit #219102): rehidrata descripción + archivos ya subidos con toast "Recuperamos tu borrador sin enviar"; el Enviar posterior vincula los archivos sin re-subirlos. La clase de bug pasa de "pérdida silenciosa" a "recuperable".
3. **Red de seguridad server-side:** cron horario (:22) `gestor_uploads_alertar_huerfanos()` — objetos del bucket >2h sin referencia en `tracking_lineas` → `notify_all_gerentes` (campanita + email) con trámite resuelto por token, dedupe persistente en `gestor_uploads_huerfanos_alertados`. **v2 (§6):** supresión por actividad posterior del mismo trámite (adjunto quitado por el gestor o por moderación = descarte a conciencia, dedupe sin aviso) y atribución correcta del segundo writer (`tracking-<id>/` de gerencia → copy neutro, jamás "la gestoría cargó").

**Aprendizajes:** (a) todo flujo "subir ahora, confirmar después" necesita las 3 capas: señal honesta de estado, borrador persistente y detector de huérfanos — el tilde verde de un upload NO es un envío; (b) un detector de huérfanos sin supresión por descarte deliberado genera falsos positivos que acusan en falso a terceros (la §6 encontró 3 vías: quitar adjunto, moderación que edita archivos, segundo writer del bucket); (c) el comentario del service decía `/acceso/:token` pero la ruta real es `/externo/:token` — los comentarios de rutas se verifican contra App.tsx.

**Verificación:** §6 12 agentes + refutación (8 confirmados, 8 cerrados; 2 descartados con racional). E2e rollback: v1 (detecta+notifica 2 gerentes+dedupe+idempotente) y v2 (alerta gestoría / suprimido / alerta staff con copy neutro). Live en Vercel con token QA: tipeo→draft persistido, adjunto real→banner, **recarga→recuperación completa** (el gesto del incidente), envío→línea en moderación + campanita×2, 0 mails reales despachados (neutralización UPDATE-solo). Limpieza a 0 por patrón genérico + corrida final del detector = 0. Huérfano real de Spotti borrado vía storage API.

## E-GG-178 · Cobranza "Sin descripción" al re-imputar desde el drawer + extracto con prefijo duplicado (caso Drozd) (2026-08-12)

**Síntoma (reporte JL):** revirtió una cobranza por caja equivocada y la re-imputó desde el comprobante en la Cta. Cte. — el asiento nuevo quedó "Sin descripción" en la caja. **La contabilidad estaba perfecta** (mismo comprobante, imputación e importes correctos, MP neteada, FundPlata acreditada): el defecto era solo el texto.

**Causa raíz:** asimetría entre superficies. El wizard de emisión compone `Cobranza · <renglón>` client-side; el drawer "Registrar cobranza" (re-imputaciones) tenía el campo libre y vacío → el service mandaba `''` → la RPC lo convertía en NULL. Nadie escribe la descripción a mano cuando está apurado.

**Fix (migs 0416-0418 + 3 componentes):**
1. **Mig 0416 — default server-side (fuente única de verdad):** `registrar_cobranza_comprobante` compone `Cobranza · <primer renglón>` cuando la descripción viene vacía (fallback `Cobranza · X 0001-…` sin renglones; lo escrito a mano se respeta). Data-fix del asiento de Drozd — único NULL de toda la BD, verificado.
2. **Mig 0417 — hallazgo derivado, pre-existente:** el extracto de Cta. Cte. (gerencia Y portal del cliente — `cliente_ctacte_extracto` delega) componía `'Cobranza' · descripcion` → TODAS las cobranzas del wizard se veían **"Cobranza · Cobranza · Curso…"**. Strip anclado del prefijo al componer (la descripción guardada no cambia).
3. **Mig 0418 (§6):** `fz_identificar_movimiento` rama cliente ya no puede dejar NULL (el modal de identificación en modo cliente ni tiene el campo): mismo default por comprobante + `Pago identificado` como último recurso; guard de la rama casa intacto. Regexp del extracto endurecida (`^[Cc]obranza(\s*·\s*|\s+|$)`: cubre "Cobranza" sin interpunto sin tocar "Cobranzas varias").
4. **Frontend:** `PanelComprobanteCobranza` deja de pisar el default con el genérico `'Cobranza desde solicitud'` (que además habría duplicado el prefijo); placeholders "Si lo dejás vacío, se completa solo…" en drawer y panel; **hint del drawer corregido** — decía "Solo visible en la caja" pero la descripción se ve en el extracto del cliente y en la rendición del partner (riesgo de notas internas expuestas).
...
## E-GG-204 · ALTO: cualquier cliente logueado podía mandar email arbitrario desde la casilla de la empresa (send-comprobante-email) (2026-09-14, auditoría C2b)
- **Síntoma:** hallazgo de la revisión adversarial de la auditoría (no reportado por usuario). La edge fn `send-comprobante-email` envía desde `contacto@gestionglobal.ar` vía Gmail API.
- **Causa raíz:** el único control era que el caller pudiera LEER el comprobante (RLS bajo su JWT); NO se miraba el rol. Los destinatarios (`to`/`cc`/`bcc`), asunto y PDF venían del caller. Un cliente del portal (role `administrador`) podía invocarla con `to` arbitrario → **usar la casilla de la empresa para phishing/spam a terceros**, salteando el throttle de la cola. El modal `EnviarComprobanteModal` se usa en gerencia (facturación) Y en el portal del cliente, así que un staff-gate simple habría roto el flujo legítimo del cliente.
- **Fix (edge fn v18; decisión de Pablo: restringir destinatarios del cliente):**
  1. Auth-gate por ROL (`getUser` + `profiles.role`): staff (gerente/operador) sin límite; cliente (no-staff) SÓLO puede enviar a casillas ACTIVAS de SU administración (`administracion_emails`, que es **staff-managed** — INSERT/UPDATE/DELETE gateados por `is_staff()`, así que el cliente no puede inyectar direcciones) + su propio email de acceso **confirmado** (`email_confirmed_at`). Fuera de eso → 403.
  2. Cuerpo SIEMPRE server-render (se ignora `payload.html`).
  3. **Anti-inyección MIME** (hallazgos de la §6 sobre el propio fix): se valida CADA `to`/`cc`/`bcc` contra CR/LF + caracteres de control + formato de email, para TODOS (incluido staff), ANTES de armar el MIME (cierra CRLF header-injection que un staff podía usar para colar un `Bcc:` externo); se unifica el dato validado con el enviado; y `pdf_filename` se sanea (strip CR/LF/comillas/backslash/control) antes de interpolarlo en `name=`/`filename=` (cerraba una inyección de parte MIME/HTML de phishing por parte del cliente).
- **Prueba (v18, live):** cliente→externo 403; cliente→casilla permitida/su propio login pasa; cc/bcc externo 403; mayúsc+espacios normalizados (sin bypass); gerente→externo pasa; comprobante ajeno 404; anon/sin-auth 401; PDF inválido 400; **CRLF en to/cc (staff y cliente) 400**; email malformado 400; **filename malicioso → saneado** (verificado en `sent_emails`: sin CR/LF/comillas, y el email real llegó limpio, sin parte inyectada). Envío real cliente→casilla permitida con PDF válido = 200 + render de marca OK (confirmado por Pablo). QA efímero (admin+cliente+gerente+comprobante) borrado a 0.
- **Prevención:** ninguna fn que envíe email debe confiar en destinatarios/nombre-de-archivo del caller sin validar CR/LF + formato; para superficies compartidas gerente↔cliente, gatear por ROL (no bloquear el flujo del cliente, restringir su alcance).
- **Pendiente (defensa en profundidad, documental):** la inclusión de `caller.email` en el allow-list depende de que Supabase Auth exija confirmación al cambiar de email ("secure email change" ON).
- **Fecha / módulo:** 2026-09-14 · edge functions / facturación / email / seguridad.

## E-GG-205 · MEDIO: doble-click/reintento podía imputar un pago DOS veces (RPC de dinero sin idempotencia) (2026-09-14, auditoría A-INTEG)
- **Síntoma:** hallazgo de la auditoría (A-INTEG). Un doble-click o un reintento de red al registrar una cobranza o un pago de curso podía crear DOS asientos de ingreso por el mismo pago.
- **Causa raíz:** `pago_conciliar` ya era idempotente (máquina de estados). Pero `registrar_cobranza_comprobante` sólo tenía protección parcial (FOR UPDATE + chequeo de saldo → un pago PARCIAL con saldo suficiente se duplicaba) y `curso_registrar_pago` no tenía ninguna guarda.
- **Fix (decisión de Pablo = clave de idempotencia desde el front; único enfoque sin falsos positivos):**
  - **Backend (mig 0479):** columna `movimientos.idempotency_key uuid` + índice ÚNICO PARCIAL (`WHERE idempotency_key IS NOT NULL`). Las 2 RPC (DROP+CREATE, R16) reciben `p_idempotency_key uuid DEFAULT NULL` (último param → no rompe el caller posicional de `pago_conciliar`). Lógica: si el key ya existe → devolver el movimiento existente (no duplica); si no, insertar con el key; `EXCEPTION WHEN unique_violation` → devolver el existente (carrera, misma intención) o RAISE. `key NULL` = comportamiento idéntico al anterior (sin dedup). Grants: authenticated+service_role; PUBLIC+anon revocados (default de Supabase re-agrega anon → revocado explícito, T1).
  - **Front:** hook `useIdempotencyKey` (clave ESTABLE por montaje + `renew()` **sólo tras éxito** → un doble-click/reintento de la misma intención reusa el key (deduplica); dos pagos legítimos distintos usan claves distintas (NUNCA falso positivo)). Cableado en los 3 flujos standalone de cobrar-un-comprobante/pago-de-curso (RegistrarCobranzaDrawer, PanelComprobanteCobranza/ModalRegistrarPago, RegistrarPagoModal) + en el wizard `ProcesadorFinal` (que además ya tenía guarda propia check-then-act). Los flujos "emitir+cobrar" quedan fuera a propósito (su doble-submit crea otro COMPROBANTE, no una cobranza duplicada; guardado por botón deshabilitado).
- **Prueba:** e2e backend con rollback (mismo key → 1 movimiento + 1 imputación; keys distintos → 2; NULL → sin dedup; replay devuelve el original; ambos RPC). Revisión adversarial §6 (3 lentes: backend-race / front-falso-positivo / cobertura-regresión) → CERRADO_SIN_REGRESION en las 3, sin hallazgos crítico/alto/medio. Hallazgos BAJA/INFO atendidos: fallback UUID del hook hecho v4 válido; key agregado a `ProcesadorFinal` para cerrar su carrera check-then-act. Verificado que PostgREST resolvió la firma nueva (schema cache) con el shape viejo (front actual) y el nuevo → sin romper cobranzas en vivo. `tsc` verde.
- **Prevención:** todo RPC que cree dinero y sea alcanzable por doble-click debe aceptar una idempotency-key del front (una por intención). Otros RPC creadores de dinero fuera de scope quedan anotados como deuda (mismo patrón aplicable).
- **Fecha / módulo:** 2026-09-14 · finanzas / RPC / front · A-INTEG.

## E-GG-206 · MEDIO→BAJO: gmail-pubsub-webhook permitía a anon falsear el estado de emails (2026-09-15, auditoría)
- **Síntoma:** hallazgo del barrido de edge functions. La fn `gmail-pubsub-webhook` es un stub (el camino real de Google Pub/Sub es un no-op ACK, nunca cableado).
- **Causa raíz:** un camino de "testing manual" aceptaba, SIN auth (verify_jwt=false, anon-callable), un payload `{provider_msg_id, event}` y actualizaba `sent_emails.webhook_status`/`last_event_at`. Cualquiera podía marcar cualquier email como entregado/rebotado. **Impacto real bajo:** `webhook_status` no se lee en ninguna pantalla ni lógica, y nada del sistema invoca esta fn (verificado por grep).
...
## E-GG-208 · El recordatorio de comprobantes impagos nunca envió un solo email (cron 401 en silencio)

- **Síntoma:** el cron `notify-vencimientos-diario` (jobid 2) figuraba "succeeded" en `cron.job_run_details` (120 corridas), pero la feature de recordatorios de facturas impagas a las administraciones NUNCA envió nada: `comprobante_avisos_vencimiento` tiene 0 filas en toda la vida de producción.
- **Causa raíz:** el comando del cron mandaba un `Authorization: Bearer myRhvg…` HARDCODEADO y VIEJO (drift R7: el repo 0011 lo definía con `current_setting(app.settings.cron_secret)`, pero el job vivo fue re-agendado a mano con un token que nunca coincidió con `CRON_SECRET`) → la edge `notify-vencimientos` respondía 401 en cada corrida. Como `net.http_post` es fire-and-forget, pg_cron marca 'succeeded' al ENCOLAR, no según el status HTTP → falso-verde total (hallazgo A6 de la auditoría). Nadie lo notó porque la feature nunca había funcionado (no había línea base con la cual comparar).
- **Fix (mig 0496 · Fase C/C2):** se RETIRÓ el cron roto (`cron.unschedule`) para matar el falso-verde, y se des-cegó el monitoreo: `db_health_metrics()` ahora cuenta las respuestas HTTP 4xx/5xx/timeout de 24h y las alerta. **NO es un duplicado** de `dispatch-vencimientos` (ese cubre vencimientos regulatorios sobre la tabla `vencimientos`, no comprobantes). **DECISIÓN DE PRODUCTO ABIERTA:** reactivar los recordatorios de facturas impagas (re-alinear el token a `private.cron_bearer()` + validar el contenido/umbrales) mandaría emails nuevos a ~79 clientes que nunca los recibieron → requiere el OK de Pablo; no se enciende de forma autónoma. Infra conservada por si se reactiva: edge `notify-vencimientos`, vista `vw_comprobantes_para_avisar`, tabla `comprobante_avisos_vencimiento`.
- **Prueba:** cron retirado (0 en cron.job; dispatch-vencimientos real sigue activo); `db_health_metrics()` como gerente devuelve `cron_http_fallas_24h` y la alerta `cron_http`. Falso-verde eliminado.
- **Fecha / módulo:** 2026-09-16 · crons / observabilidad / R7 drift.

## E-GG-209 · El link "Ver comprobante" del detalle de trámite nunca aparecía (columna muerta)

- **Síntoma:** en el detalle de un trámite (gerencia, `TrackingDetailPage`) el link "Ver comprobante" no aparecía aunque el trámite tuviera comprobante emitido; y el atajo "comprobante pendiente" del header se mostraba en trámites que YA tenían comprobante.
- **Causa raíz:** `tramites.comprobante_id` está muerta (0/125 filas): los comprobantes se emiten por el wizard de admisión y quedan en `solicitudes.comprobante_id` (125/125). `getTracking` embebía el comprobante por el FK de `tramites` (`comprobante:comprobantes(...)`), que daba null. El computed field `comprobante_pendiente` (función PostgREST sobre `tramites.comprobante_id`) también daba TRUE por lo mismo. (El modal de emisión desde el trámite sí setea `tramites.comprobante_id`, pero ningún comprobante en prod se emitió por esa vía.)
- **Fix (front-only, Fase D/D2):** en `getTracking`, si el embed no trae comprobante, resolverlo por la solicitud del trámite (`solicitudes.tramite_id → comprobante_id → comprobantes`) y setear `comprobante_pendiente=false`. `getTracking` sólo lo usa la gerencia → RLS OK; para el cliente el fallback da null (sin regresión). No se tocó la BD (se descartó backfill+trigger por blast radius sobre 125 filas vivas para un bug 🟢). El duplicado nunca ocurrió: el modal es idempotente sobre `solicitud.comprobante_id`.
- **Prueba:** el fallback resuelve los 125 trámites en BD (ej. "X 0001-00000125"); prueba en vivo del link en el detalle tras deploy.
- **Fecha / módulo:** 2026-09-16 · trackings / comprobantes / SSOT de dominio.

## E-GG-210 · La cobranza/recupero AUTOMÁTICO nunca corrió (cron 401 diario por falta de bearer)

- **Síntoma:** el métrico C2 (`cron_http_fallas_24h`, banner en *Salud del sistema*) marcó **3 fallas HTTP en 24 h**. Al abrirlas: **1 × 401** `{"ok":false,"error":"unauthorized"}` a las 12:30 UTC (diario) + **2 × timeout 5000 ms** (11:00 y 11:02 UTC).
- **Causa raíz (el 401):** el cron `dispatch-recupero-diario` (jobid 8, `30 12 * * *` = 09:30 AR) hace `net.http_post` a la edge `dispatch-recupero` **SIN header `Authorization`** — es el ÚNICO `http_post` cron que no manda `private.cron_bearer()` (todos los demás dispatchers sí). La edge tiene `verify_jwt=false` pero chequea `Bearer CRON_SECRET` internamente (CRON_SECRET está seteado) → devuelve 401 **antes** de correr. `dispatch_recupero_log` = **0 filas en toda la historia de prod** (el 401 corta antes del insert al log) → el recupero automático **nunca encoló un solo email de dunning**. Lección §6 (revisar-sin-ejercitar-el-camino-real): E-GG-129 (mig 0350) arregló una capa INTERNA (el assert de tenencia para `service_role`) y lo verificó e2e vía RPC-con-JWT del gerente, **pero nunca ejercitó el camino HTTP real del cron** → quedó un 2º breakage EXTERNO (el bearer) sin detectar. El circuito MANUAL sí anda: gerencia dispara recupero a mano vía `disparar_recupero_manual` (DispararRecuperoDrawer); el subsistema `src/modules/recupero/` es real y completo.
- **Causa raíz (los 2 timeouts):** dispatchers por-minuto con el timeout default de pg_net (5000 ms); uno es un timeout puro de DNS (5001 ms de resolución). Blips transitorios de red/infra, benignos (los dispatchers son idempotentes + la cola persiste → la corrida del minuto siguiente reintenta). No requieren acción.
- **DECISIÓN DE PRODUCTO ABIERTA (no se enciende autónomamente):** arreglar el cron (agregar `private.cron_bearer()` al jobid 8, como todos los demás) activaría, **por primera vez**, dunning automático a los ~17 morosos reales en vivo (algunos 20-34 días vencidos). No es un cambio quirúrgico/aditivo — dispara emails a usuarios reales → requiere el OK de Pablo (revisar config de niveles R1/R2/R3, plantillas, timing). Gemelo de E-GG-208 (notify-vencimientos) y [[reference_arca_dormido_tipo_x]]. Infra conservada. Lo cazó el métrico de C2 → validación de que la observabilidad agregada sirve.
- **Prueba:** read-only. `dispatch_recupero_log` 0 filas; `net._http_response` 24 h muestra el 401 de las 12:30 + 2 timeouts; `cron.job` confirma que jobid 8 es el único `http_post` sin `Authorization`; `get_edge_function('dispatch-recupero')` confirma el chequeo `Bearer CRON_SECRET` interno. Sin cambios en prod.
- **Fecha / módulo:** 2026-09-17 · crons / cobranza / observabilidad · lección §6 (ejercitar el camino real).

## E-GG-211 · saldo_pendiente quedaba stale al editar ítems de un comprobante ya cobrado (mora subvaluada)

- **Síntoma:** hallazgo de la auditoría de mora (DGG-186). `recalcular_totales_comprobante()` (trigger AFTER I/U/D en `items_comprobantes`) recalculaba `saldo_pendiente` SOLO cuando `estado_cobranza='pendiente'` (`CASE WHEN 'pendiente' THEN sub.total ELSE saldo_pendiente END`). Si se editaban los ítems (cambia `total`) de un comprobante ya `parcial`/`pagado`/`vencido`, el saldo quedaba VIEJO → mora mal informada al gerente hasta el próximo evento de imputación (que sí lo auto-cura vía `recalcular_saldo_comprobante_imputado`).
- **Causa raíz:** dos triggers mantienen el saldo con distinta fórmula: el de imputaciones (`recalcular_saldo_comprobante_imputado`) ya usaba la canónica `GREATEST(0, total − imputado)`, pero el de ítems tenía un short-circuit que solo lo tocaba en estado 'pendiente'. Latente: hoy 0 comprobantes en drift (la reconciliación libro↔saldo dio 0 desviaciones en 127), pero un edit de ítems sobre un cobrado parcial lo dispararía.
...
## E-GG-213 · El pago de curso en CUOTAS nunca completaba → certificado retenido (M-CAMPUS, mig 0499)

- **Síntoma (latente, 0 víctimas hoy):** hallazgo de la auditoría (M-CAMPUS) confirmado por §6-revisar. `curso_registrar_pago` marcaba la matrícula 'pago_completo' (y cumplía la condición 'pago' → habilita certificado) sólo si el monto de UN pago alcanzaba el precio. Un curso con precio real pagado en cuotas (p.ej. 4000+6000 de 10000) quedaba en 'pago_parcial' PARA SIEMPRE → certificado retenido injustamente.
- **Causa raíz:** `v_completo := p_monto >= v_curso.precio_lista` usaba el monto del pago individual, no el ACUMULADO de la matrícula. Además `movimientos` no tenía vínculo a la matrícula (SSOT del pago desacoplado del dinero). No mordía hoy porque el único curso con matrículas parciales tiene `precio_lista` NULL (auto-completa) y los cursos con precio ($80.000) ya estaban todos 'pago_completo'.
- **Fix (mig 0499, aditivo, sin backfill riesgoso):** (1) `movimientos.matricula_id uuid REFERENCES curso_matriculas ON DELETE SET NULL` (SSOT; el `SET NULL` preserva el dinero si se borra la matrícula) + índice parcial R11. (2) `curso_registrar_pago` (CREATE OR REPLACE, misma firma → R16 OK) setea `matricula_id` y deriva `v_completo` del `SUM(monto) de movimientos de la matrícula (tipo='ingreso', estado NOT IN anulado/revertido) >= precio - 0.009`. (3) `FOR UPDATE` sobre la matrícula → serializa pagos concurrentes (SUM siempre exacto). (4) branch parcial con guard `NOT IN ('pago_parcial','pago_completo')` → NUNCA degrada una completa.
- **Prueba (EJERCITADO e2e con ROLLBACK forzado, 2×):** con precio temporal 10.000 sobre una matrícula real: adeudado → pago 4000 = 'pago_parcial' (NO completa) → pago +6000 = 'pago_completo' (acumulado 10.000), 2 movimientos linkeados, sum 10.000. Rollback verificado sin rastro (matrícula sigue parcial, precio sigue NULL, 0 linkeados). **§6: 3 agentes adversariales (lógica / SSOT-downstream / integridad-financiera-seguridad) → 0 hallazgos crítico/alto**; certificado se habilita bien y no de más (gatea por `matricula_condiciones.cumplida`, no por estado_pago); idempotencia E-GG-205 intacta; RLS/grants sin exposición nueva; audit trigger SECURITY DEFINER (R17) captura la columna nueva.
- **GAPs pre-existentes documentados (NO introducidos por 0499; diferidos, requieren decisión de diseño/negocio):**
  1. **Doble-SSOT de `estado_pago`:** además de este path (movimientos), `private.matricula_estado_pago_desde_comprobante` (trigger `trg_comprobante_sync_matricula_pago`) escribe estado_pago derivándolo de `comprobantes.saldo_pendiente`. Para una matrícula con comprobante+trámite Y pagos de campus pueden discrepar (el path de comprobante no tiene guard anti-degradación). Unificar el SSOT de pago de campus es una decisión de diseño (fuera de scope de 0499).
  2. **Sin backfill de `matricula_id` histórico** (107 movimientos de campus quedaron NULL): una matrícula "a caballo" (pago viejo sin linkear + pago nuevo) sub-contaría. 0 instancias hoy (los priced ya completos). Un re-link/import histórico debe re-vincular movimientos o setear estado a mano.
  3. **Anular/revertir un pago completo NO recomputa estado_pago hacia abajo** (el guard nunca degrada + la reversa no pasa por esta RPC) → matrícula queda completa con plata revertida. Pre-existente. Cerrarlo = trigger en movimientos (anulado/revertido) que recalcule el acumulado.
- **RESUELTO (investigación §6 + confirmación de Pablo):** el "Curso Integral de formación..." NO es gratis y NO le falta precio: se cobra **100% por COMPROBANTE** (no por `curso_registrar_pago`), con precio **por-alumno que varía 300k–360k** → por eso `cursos.precio_lista` es NULL y es correcto (el precio vive en cada comprobante, no en una lista fija). Las 15 matrículas `pago_parcial` son **cuotas legítimas** (el alumno abona en 2 veces) manejadas correctamente por el path comprobante (saldo>0 → parcial; completan al saldar la 2ª cuota). **0 impacto en certificado:** las 26 matrículas del curso NO tienen condición 'pago' (el cert no se gatea por pago acá). Barrido global: 74 matrículas con condición 'pago' (3 cursos, modo campus-movimiento) → **todas cumplida, 0 certificados trabados, 0 víctimas**. Mi fix 0499 aplica al modo campus-movimiento (donde el bug mordía a futuro); este curso no lo usa → no se ve afectado. **NO requería acción; no se tocó ningún dato.**
- **GAP simétrico registrado (latente, decisión de diseño, NO tocado):** el path comprobante (`matricula_estado_pago_desde_comprobante`) setea `estado_pago` pero NO marca la condición 'pago' de `matricula_condiciones`. Si algún día un curso cert-gated (con condición 'pago') se cobrara por comprobante, el cert quedaría trabado. Hoy 0 instancias (los 3 cert-gated se cobran por campus-movimiento; el comprobante-based no tiene condición 'pago'). Cerrarlo = que el path comprobante marque la condición al saldar (agrega emisión de cert al flujo de comprobante) → decisión de Pablo.
- **Fecha / módulo:** 2026-09-17 · campus / pagos / SSOT del dinero · M-CAMPUS.

## E-GG-214 · La capa "confirmado por trámite" leía `estado='cerrado'`+`fecha_fin` cuando el terminal real es `resuelto`+`resuelto_at` (Agenda Fase 1)

- **Síntoma (latente, corregido antes de exponer al cliente):** la RPC `perfil_regulatorio_get` (mig 0500/0501) derivaba la certeza "confirmado" de cada hecho (última renovación/curso/DDJJ/certificado/consultoría + matriculado) filtrando trámites por `t.estado='cerrado'` y datando con `max(t.fecha_fin)`. Pero el estado TERMINAL operativo del sistema es **`resuelto`** (108 filas en prod, `fecha_fin` SIEMPRE NULL; `resuelto_at` poblado 108/108); sólo `tracking_cerrar` produce `cerrado`+`fecha_fin` (2 filas en toda la BD). ⇒ la capa "confirmado por trámite" estaba **prácticamente muerta**: el perfil caía a `inferido`/`desconocido` justo cuando el hecho estaba confirmado por la plataforma — exactamente lo contrario del propósito de la feature ("nunca afirmar una presunción / la plataforma es la fuente CONFIRMADA").
- **Causa raíz:** al escribir la RPC asumí `cerrado` como estado terminal (el intuitivo), sin verificar el ciclo de vida real de `tramites`. El estado terminal es `resuelto` (lo estampa el trigger vivo `tramite_on_update`, que trata `estado IN ('resuelto','cerrado')` como resuelto y setea `resuelto_at`, no `fecha_fin`). Es un dato que YA conocía de un chunk anterior (M-MODERA: "0 trámites cerrados, terminal es resuelto") y aun así lo repetí — el sesgo "revisar lo que escribí" no lo ve; lo cazó el EJERCITAR e2e del §6.
- **Detección:** doble auditoría §6 obligatoria de cierre — **DOS agentes independientes** (persistencia + integración) lo encontraron y lo verificaron e2e (un admin con renovación resuelta el 2026-08-12 → la query devolvía NULL).
- **Fix (mig 0502, `CREATE OR REPLACE` misma firma → R16 OK):** las 6 subqueries usan `t.estado IN ('resuelto','cerrado')` + `max(COALESCE(t.fecha_fin, t.resuelto_at::date))` + gate `t.cierre_satisfactorio IS NOT FALSE` (no contar un trámite frustrado como logro). En el mismo pase se corrigieron los otros hallazgos del §6: DDJJ derivada del 31/03 etiquetada `inferido` (no `confirmado`) y gateada a jurisdicción RPAC; `completitud_pct` cuenta sólo confirmado+declarado (no infla con inferidos); se agregó `proximo_certificado` (+3m); `curso_rpa_caba` incluido en la búsqueda de vencimientos sembrados; grant de tabla bajado a SELECT (write sólo por RPC definer, least-privilege).
...
- **CERRADO (DGG-197, mig 0504):** el form del cliente (`/portal/mi-ficha`) tiene switches de opt-out que llaman `perfil_regulatorio_set_no_requiere` (RPC de REEMPLAZO wholesale → permite quitar claves). `no_requiere` ahora tiene writer; R14 cubierto en la superficie del cliente. (El editor de gerencia sigue sin tocar `no_requiere` a propósito: el opt-out lo decide el cliente, no gerencia.)
- **Fecha / módulo:** 2026-09-22 · perfil regulatorio · Agenda progressive profiling (DGG-196 → cerrado DGG-197).

## E-GG-215 · Modo sombra: el seed CABA contaminaba el espacio de gracia/dedup (sub-proyectaba ofertas)

- **Síntoma (fidelidad de la simulación, no envío real):** el motor **sombra** (`gg_ofrecimientos_diario_sombra`, DGG-199) escribía la intención `caba_venc` de §0 en `ofrecimientos_sombra` — la MISMA tabla que gobierna la supresión de ofertas (la gracia 7d de §2: `CONTINUE WHEN EXISTS ... ciclo_ancla >= hoy-7`, sin filtrar `codigo`; y el dedup diario de §1). En el motor REAL, §0 escribe en `vencimientos` (tabla APARTE de `ofrecimientos_log`), así que esos admins NO se suprimen. En la sombra sí → los 2 admins de la cohorte CABA (Sanclaudio, Baldonccini) quedaban **suprimidos permanentemente** de las cadencias en la simulación. Además, como la sombra nunca crea el `vencimiento` real, el guard `NOT EXISTS (... FROM vencimientos)` de §0 quedaba siempre-true → se re-insertaba un `caba_venc` cada día (gracia eterna + `caba_seed_intencion` del reporte sobrecontado: 2 aniversarios se veían como ~28 en 14 días).
- **Causa raíz:** mezclar el seed de datos (CABA) con las señales de supresión de comms en la misma tabla, cuando en el motor real viven en tablas distintas. El día-1 no cambiaba (igual se llega al cap 40) pero la CURVA multi-día que Pablo iba a leer para decidir el encendido quedaba sesgada, y justo en la cohorte CABA.
- **Detección:** doble auditoría §6 del modo sombra (agente de aislamiento) — el aislamiento crítico (NO envía nada real) dio limpio; esto es un bug de fidelidad de la simulación.
- **Fix (mig 0509):** (a) excluir `AND s.codigo <> 'caba_venc'` de la gracia §2 y el dedup diario §1 → el seed CABA nunca suprime cadencias; (b) sembrar `caba_venc` con `ciclo_ancla=aniversario` (estable) + dedup del §0 contra la propia sombra → se registra una sola vez por aniversario; (c) portar el filtro `webinar_inscriptos` a §1 (paridad). Verificado: los 2 admins CABA ahora quedan elegibles a cadencia (elig_cert=true) y no se re-insertan. Se borró el día-1 buggy y se re-corrió limpio.
- **Fecha / módulo:** 2026-09-23 · motor de ofrecimientos / modo sombra · Agenda (DGG-199).

## E-GG-216 · Alineación del motor a las 6 consignas: contrato-vs-código en C2 + GAP R14 matriculación (caza §6)

Todos LATENTES (0 admins afectados hoy, motor real DORMIDO) — cazados por la doble auditoría §6 (3 agentes) de las migs 0510/0511 ANTES de cualquier envío, y corregidos en 0512.

- **C2 gateado por número en vez de por vencimiento (A#1/A#2):** las reglas `curso_venc`/`renovacion_venc` exigían `matricula_conocida` (número), pero la consigna C2 dice "ofrecer cuando se conoce el VENCIMIENTO". Un admin con vencimiento/emisión conocida pero sin número quedaba excluido de curso Y renovación (y como `es_rpac` sin número también bloquea cert/ddjj, caía sólo a consultoría). **Fix:** curso/renov gatean por `prox_venc IS NOT NULL` (vencimiento conocido); cert/ddjj SIGUEN por número (eso es C1); `matriculacion` suma `prox_venc IS NULL` para no ofrecerse a quien declaró una fecha.
- **Renovación urgente saltada (A#4):** con precedencia curso-primero + gracia 7d, un venc ≤7d podía pasar antes de que la renovación saliera (el aniversario rodaba a +12m y salía de ventana). **Fix:** curso sólo dispara con venc >15d; el inminente va directo a renovación.
- **prox_venc ocultaba matrículas VENCIDAS (C-H1):** el helper rodaba un vencimiento ya pasado al año siguiente → un matriculado lapsado (ej. Padrón César, vencido hace 23d) se veía como "renueva el año que viene" y NO recibía renovación (¡el caso más urgente!). **Fix:** ventana overdue 60d en el helper + `renovacion_venc` cubre `[hoy-60, hoy+45]`. Padrón ahora entra a renovación.
- **prox_venc en UTC (C-H2):** usaba `CURRENT_DATE` (=UTC) mientras el motor usa fecha AR → off-by-one en ventana nocturna. **Fix:** el helper usa `now() AT TIME ZONE 'America/Argentina/Buenos_Aires'`.
- **Aniversario fin-de-mes (C-H5):** `LEAST(dia,28)` adelantaba todo fin de mes. **Fix:** usa el último día real del mes.
- **R6 least-privilege (B#1):** los helpers `private.gg_admin_matricula_conocida` / `gg_admin_prox_venc_matricula` (y los pre-existentes `gg_admin_es_*`) tenían PUBLIC EXECUTE. **Fix:** `REVOKE EXECUTE ... FROM PUBLIC` (igualados a `gg_ofrecimiento_elegible`).
- **GAP R14 · `no_requiere.matriculacion` sin writer:** `elegible`/`preview`/`ClientePanoramaCard` leen la clave `no_requiere.matriculacion` (tile "No lo requiere"), pero NINGÚN control del form del cliente la escribe. **Decisión (no se cierra ahora):** el escape "ya estoy matriculado" es cargar el número (flipea `matricula_conocida` → sale de matriculación). El guard backend queda como future-proof. Si aparece el caso "no ejerzo / nunca me voy a matricular", agregar un opt-out `matriculacion` en `fichaModel.ts` (OBLIGACIONES) + el form. Motor dormido → baja prioridad.
- **CAZADO EN VIVO (mig 0513) · E-GG-214 recurrente en `gg_admin_es_rpac`:** durante el live-test del panorama (gerente QA efímero), Catelli Federico (inscripción RPAC RESUELTA, matrícula otorgada) mostraba "Matriculación · Le corresponde" — se le ofrecía matricularse pese a estar matriculado (viola C3). Causa: `gg_admin_es_rpac` contaba trámites sólo con `estado='cerrado'`, pero el terminal es `resuelto` (E-GG-214) → la rama de trámites estaba muerta (0 filas 'cerrado'), es_rpac dependía sólo del número en ficha. 4 admins con inscripción resuelta sin número quedaban mal targeteados. **Fix (0513):** contar trámites RPAC `<> 'cancelado' AND cierre_satisfactorio IS NOT FALSE` (mismo criterio robusto que el hermano `gg_admin_es_solo_caba`, inmune a resuelto/cerrado). Único caller: el motor (real dormido + sombra). Post-fix: matriculación 33→26 (los matriculados-sin-número salen a consultoría); Catelli deja de recibir matriculación. Esto es EXACTAMENTE por qué el live-test es ineludible: la doble auditoría §6 estática no lo vio (todos los agentes trataron `gg_admin_es_rpac` como caja negra correcta); lo cazó ver el panorama real de un cliente real.
- **Lección / regla:** cuando una consigna define el disparador ("cuando se conoce el vencimiento"), la elegibilidad debe gatear por ESE dato (prox_venc), no por un proxy más estricto (número). Y toda función de fecha derivada debe (a) fijar timezone AR explícito, (b) contemplar el caso "vencido/pasado", (c) usar el último día real del mes. La doble auditoría §6 (revisar + EJERCITAR) cazó 6 hallazgos antes de prod; el 7º (es_rpac) lo cazó el live-test. Los helpers pre-existentes reusados también deben auditarse contra E-GG-214 (¿usan 'cerrado' donde el terminal es 'resuelto'?).
- **Fecha / módulo:** 2026-09-24 · motor de ofrecimientos · Agenda (DGG-200, migs 0510-0513).

## E-GG-217 · "Alarmas de hoy" no notificaban (ni campanita ni email): desfase de timezone widget↔cron

- **Reporte:** JL (doc "Sistema Gestion Global", tab PARA VER:, 23/9/2026): el widget "Alarmas de hoy · 2 para hoy" (Inscripción RPAC · Rosa Matías Hernán TRM-2026-00165 + Consulta jurídica · Duro Camila TRM-2026-00166) no disparó email ni campanita.
- **Causa raíz:** el widget `public.gerencia_alarmas_hoy()` corre con `SET "TimeZone"='America/Argentina/Buenos_Aires'`, pero el cron que NOTIFICA (`private.dispatch_alarmas_tracking_hoy()`, pg_cron jobid 14, 09:00 BA) NO fijaba timezone → corría en UTC. Una alarma con `alerta_en` en horario nocturno BA (≈00:xx UTC) se ve en el widget el día X (BA) pero el cron la considera del día X+1 (UTC) y no la despacha. En ese hueco JL resolvió una y postergó la otra → `alarma_dispatched_at` quedó NULL y nunca se emitió `tracking_alarma`. Bug intermitente desde el día 1 (dependía de la hora del `alerta_en`); las alarmas con `alerta_en` diurno BA sí notificaban (17 líneas despachadas OK en la historia).
- **Fix (mig 0514):** agregar `SET "TimeZone"='America/Argentina/Buenos_Aires'` al dispatcher (cuerpo idéntico) → `CURRENT_DATE`, `alerta_en::date`, el guard `alarma_dispatched_at::date` y el flag `vencida` se evalúan en BA, byte-idénticos al widget. Sin cambio de firma (R16 OK). Backlog al aplicar = 0 (verificado, sin lote atrasado).
- **Verificación:** EJERCITAR e2e (rollback): con una alarma "hoy en BA / mañana en UTC" el dispatcher fixeado la toma (dispatched_count=1, notif_delta=2 a los 2 gerentes, línea marcada). §6 (3 agentes): paridad byte-idéntica con el widget, no toca al cliente, sin regresión. Hallazgos pre-existentes NO tocados: re-nag diario de vencidas (intencional) y falta de FOR UPDATE (cron único diario).
- **Lección:** toda función de fecha que deba coincidir con otra superficie debe fijar el MISMO timezone explícito. Un widget con tz y su cron sin tz divergen 1 día calendario en la ventana nocturna. Ver también E-GG-216 (prox_venc UTC vs AR).
- **Fecha / módulo:** 2026-09-25 · alarmas de tracking · reporte JL A1.

## E-GG-218 · Cartel "Avisá a la gestoría" colgado permanente en la ficha del trámite

- **Reporte:** JL (doc, tab PARA VER:): "pone ese cartel como si hubiese algo para avisar A La Gestoría pero no hay nada, queda colgado con la última interacción con Gestoría".
- **Causa raíz:** en `TrackingDetailPage.tsx` la condición del cartel usaba `adjuntosPedidoDoc.length > 0` — verdadero PARA SIEMPRE apenas el cliente subió UN archivo en la historia del trámite (el array trae todo ítem con archivo, sin filtrar estado) — y sólo excluía `cerrado`/`cancelado`, NO el estado terminal real `resuelto` (E-GG-214). Datos: 12 trámites abiertos/derivados mostraban el cartel con ítems 100% 'aprobado' (0 'subido'); 9 de ellos ya 'resuelto'.
- **Fix (frontend):** condición → `isStaff && derivacion && !['cerrado','cancelado','resuelto'].includes(estado) && adjuntosPedidoDoc.some(a => a.estado === 'subido')`. El cartel aparece sólo cuando hay al menos un ítem recién subido por el cliente y sin procesar; al aprobar/rechazar se limpia. **§6 (media, corregido en el mismo chunk):** `adjuntosPedidoDoc` se cargaba una sola vez por `data.id` y no se refrescaba in-session → el cartel quedaba stale tras aprobar/rechazar. Se sumó `tramite_pedidos_doc_items` al `useRealtimeRefresh` + un `adjuntosNonce` que re-fetchea los adjuntos → el cartel aparece/limpia en la misma sesión.
- **Verificación:** e2e (rollback) del predicado (fires_before_flip=f, fires_after_flip=t) + §6 (predicado nuevo = endurecimiento estricto, 12→0 falsos positivos; el botón manual de la toolbar queda como fallback). tsc verde. Hallazgos menores pre-existentes no tocados: nudge en el 1er upload (discoverability), respuestas sólo-texto fuera del array.
- **Fecha / módulo:** 2026-09-25 · trackings / pedidos de doc · reporte JL A2.

## E-GG-219 · Los adjuntos de gestoría no llegaban al cliente en pedidos de documentación

- **Reporte:** JL (doc, tab PARA VER:): en el portal del cliente, un aporte "Documentación solicitada" (pedido de docu / huellas) llegaba SIN los archivos adjuntos descargables, a varios clientes; los adjuntos "informativos" (ej. constancia de presentación) sí llegan.
- **Causa raíz (confirmada con datos: 35 líneas 'documentacion_incompleta' visibles, 0 con adjuntos; 35 'gestor_avance' visibles, 32 con adjuntos):** cuando gerencia convierte un aporte de gestoría en Pedido de documentación (Moderación → onConvertirEnPedido), la RPC `tramite_pedido_doc_crear` insertaba la línea visible al cliente con `archivos_urls='{}'` HARDCODEADO y su firma ni aceptaba adjuntos; además Moderación marca la línea original del gestor (la que tiene los adjuntos) como 'interno' (visible_cliente=false) → los archivos quedan huérfanos. El camino informativo (`tracking_moderar_gestor_avance('publicar')`) conserva `archivos_urls` en la línea visible y por eso ahí sí bajan.
- **Fix aditivo (mig 0515 + servicio + Moderación):** (A) RPC acepta `p_archivos_urls text[] DEFAULT '{}'` y lo usa en la línea visible (DROP+CREATE por R16; REVOKE PUBLIC+anon, GRANT authenticated+service_role — los DEFAULT PRIVILEGES de Supabase re-otorgan anon en toda fn nueva, hay que revocarlo explícito); (B) `crearPedidoDoc` pasa `archivosUrls`; (C) `onConvertirEnPedido` reenvía `archivos` (los adjuntos del gestor). El cliente los baja con el mismo mecanismo que los informativos (bucket privado `gestor-uploads`, firmado on-click; RLS `cliente_puede_ver_adjunto_gestor` es category-agnóstica: gatea por línea `visible_cliente` de su administración + path presente).
- **Verificación:** EJERCITAR e2e (rollback): la RPC deja los adjuntos en la línea visible (match=t). §6 (3 agentes) hizo el e2e COMPLETO: `rpc_ran_ok=true, visible_line_ok=true`, y **impersonando al cliente `client_can_download=true`** (rollback); sin overload (R16, 1 firma), grants sin anon, sin fuga cross-tenant, otros 3 callers de `crearPedidoDoc` sin regresión (default '{}'). tsc verde. **Deuda documentada (decisión de Pablo):** ~35 pedidos históricos ya enviados sin adjuntos — backfill (repoblar la línea visible desde la interna) NO aplicado (cambia historial visto por clientes); el fix es hacia-adelante.
- **Lección:** al agregar un parámetro a una RPC pública, DROP+CREATE (no CREATE OR REPLACE) + revocar anon explícito (default privileges de Supabase). Un canal de "una sola vía" (cliente sube) puede necesitar también la vía inversa (gerencia/gestoría adjunta) — mirar ambas direcciones del dato.
- **Fecha / módulo:** 2026-09-25 · moderación / pedidos de doc / portal cliente · reporte JL A3.

## E-GG-220 · "Cambiar estado del tracking" al agregar un avance no impactaba (taxonomías cruzadas)

- **Reporte (JL, 2026-09-25):** al agregar un avance al tracking (drawer "Agregar línea"), el menú "Cambiar estado del tracking" ofrecía cambiar el estado del trámite, pero no impactaba — pasó trámites a "resuelto"/"finalizado" desde ahí y el estado no cambiaba.
- **Causa raíz:** el dropdown de `AgregarLineaDrawer` se poblaba desde `tracking_estados_config` (taxonomía de estados de LÍNEA: recibido/pendiente_revision/documentacion_incompleta/enviado_gestoria/en_proceso/observado/pendiente_cliente/aprobado/finalizado/cancelado) y mandaba ese slug como `p_estado_asociado`. Pero la RPC `public.tracking_agregar_linea` sólo mapea el enum canónico de `tramites.estado` (`abierto/en_progreso/esperando_cliente/resuelto/cerrado`) en un `CASE ... ELSE estado`. De los 10 slugs del config, SÓLO `cancelado` intersecta el enum → los otros 9 caían en el `ELSE estado` (sin cambio, silencioso). Es decir: TODO cambio de estado desde ese menú era un no-op. La moderación (`ModeracionPage`) no tenía el bug porque usa la constante `TRAMITE_ESTADOS`.
- **Fix (frontend, capa de presentación):** `AgregarLineaDrawer` ahora puebla el dropdown con `TRAMITE_ESTADOS`/`TRAMITE_ESTADO_LABEL` (mismo que Moderación), filtrando `cancelado` (DGG-95) y `cerrado` (§6): ambos son cierres terminales que deben pasar por sus botones dedicados (motivo + `cierre_satisfactorio` + programación de vencimiento). El drawer ofrece sólo estados de avance (incl. `resuelto`, el caso de JL). Se quitó el prop `estados` (tracking_estados_config) del drawer + su call en `TrackingDetailPage`. La RPC NO se tocó: el flujo de cierre (`CerrarTramiteDialog`) usa `estado_asociado='finalizado'/'frustrado'` como ETIQUETA de línea y depende del `ELSE estado` (hardenear la RPC lo rompería; `frustrado` ni existe en el config).
- **Verificación:** EJERCITAR (rollback) — desde `abierto`: `p_estado_asociado='finalizado'` (config) → sigue `abierto` (reproduce el bug); `'resuelto'` (enum) → pasa a `resuelto` + trigger setea `resuelto_at`. §6 adversarial (1 agente + e2e): resuelve el reporte, sin regresiones (único caller del drawer), sin otra superficie con la taxonomía equivocada; hallazgo MEDIA (dropdown ofrecía `cerrado` salteando el diálogo de cierre) corregido en el mismo chunk. tsc verde.
- **Lección:** un mismo campo (`estado_asociado`) sirve doble propósito — etiqueta de línea Y disparador de cambio de estado del trámite — y hay DOS vocabularios (taxonomía de línea `tracking_estados_config` vs enum `tramites.estado`). Un selector de "estado del trámite" DEBE poblarse del enum, nunca del config. Deuda cosmética anotada (misma confusión): el badge de estado del trámite y `estado_asociado` en el timeline se muestran como slug crudo en algunos lugares (TrackingDetailPage:386/548/982/1317, LineaTrackingCard, LineasTimeline) — usar `TRAMITE_ESTADO_LABEL`; no es el bug reportado.
- **Fecha / módulo:** 2026-09-25 · trackings / agregar línea · reporte JL.
