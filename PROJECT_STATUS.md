...
**Última actualización**: 2026-05-22 (Campus Fase 3 · **Backbone Zoom COMPLETO** — config + migración 0047 + 3 edge functions + Event Subscriptions con URL validada por challenge HMAC y 5 eventos suscriptos. Backend listo punta a punta para clases en vivo: gerencia podrá crear meetings desde un encuentro, alumnos joinearán con SDK embebido, asistencia automática vía webhook por umbral configurable, grabación cloud auto. PRÓXIMO: capa de frontend — botón "Crear sala Zoom", embed Meeting SDK en CursoDetalle (alumno/docente/moderador), badge "en vivo", panel grabaciones. Webinars públicos (Fase 3 parte 2) y rediseño cert (último punto) quedan al final.)
**Sesión actual**: Campus Fase 3 backbone — Zoom apps creadas + activadas + scopes + secrets + Event Subscriptions (5 eventos: meeting.started/ended/participant_joined/participant_left + recording.completed) + URL del webhook validada por Zoom. Migración 0047 aplicada (curso_encuentros zoom_*, asistencia auto, log inmutable de join/leave, RPCs SD). 3 edge functions deployadas (zoom-webhook unauth + zoom-sdk-signature + zoom-meeting-create).
